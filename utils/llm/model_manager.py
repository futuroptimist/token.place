...
def _build_qwen_64k_runtime_profiles(
    ...
    *,
    ...
    batch_profile: Any = QWEN_64K_BATCH_PROFILE_DEFAULT,
) -> list[Dict[str, Any]]:
    ...
    enabled_batches = batch_order[batch_order.index(requested_batch_profile):]

    def _profile_id(precision: str, batch: str) -> str:
        if batch == 'safe':
            return {'q8': QWEN_64K_RUNTIME_PROFILE_Q8, 'f16': QWEN_64K_RUNTIME_PROFILE_F16,
                    'q4': QWEN_64K_RUNTIME_PROFILE_Q4}[precision]
        prefix = 'kv_q8' if precision == 'q8' else precision
        return f'qwen64k_{prefix}_fa_{batch}_batch'

    def _base_kwargs(batch: str) -> tuple[Dict[str, Any], Dict[str, str]]:
        kwargs: Dict[str, Any] = {}
        omitted: Dict[str, str] = {}
        values = QWEN_64K_BATCH_PROFILES[batch]
        for key, value in (('flash_attn', True), ('offload_kqv', True), ('n_batch', values['n_batch']), ('n_ubatch', values['n_ubatch'])):
            if support.get(key):
                kwargs[key] = value
            else:
    ...
    combinations = [(precision, batch) for batch in enabled_batches for precision in ('q8', 'f16')]
    combinations.append(('q4', 'safe'))
    for precision, batch in combinations:
        profile_id = _profile_id(precision, batch)
        kwargs, omitted = _base_kwargs(batch)
        ...
        diagnostics = {
            ...
            'qwen_64k_batch_profile_selected': batch,
...
def normalize_qwen_64k_batch_profile(value: Any) -> str:
    """Normalize persisted/user input without ever implicitly opting into experimental."""
...
def _next_qwen_64k_runtime_profile_index(
    ...
    active = next((profile for profile in profiles if profile.get('profile_id') == active_profile_id), None)
    if not active:
        return None
    active_precision = active.get('diagnostics', {}).get('kv_precision')
    active_batch = active.get('diagnostics', {}).get('batch_profile')
    if is_qwen_64k_memory_pressure_failure_category(failure_category):
        downshift = {'experimental': 'balanced', 'balanced': 'safe'}.get(active_batch)
        target_precision, target_batch = ((active_precision, downshift) if downshift else ('q4', 'safe'))
    elif active_precision == 'q8':
        target_precision, target_batch = 'f16', active_batch
    else:
        return None
    target = next((profile.get('profile_id') for profile in profiles
                   if profile.get('diagnostics', {}).get('kv_precision') == target_precision
                   and profile.get('diagnostics', {}).get('batch_profile') == target_batch), None)
    if not target or target == active_profile_id or target not in profile_ids:
        return None
...
class ModelManager:
    ...
    def __init__(self, config=None):
        ...
        self.last_compute_diagnostics = {
            ...
            'qwen_64k_batch_profile_selected': 'not_applied',
        }
    ...
    def _publish_qwen_64k_init_failure_readiness_diagnostics(
        ...
        *,
        ...
        diagnostics.update({
            ...
            'api_v1_readiness_qwen_64k_runtime_preferred_profile_id': QWEN_64K_RUNTIME_PROFILE_DEFAULT,
            'api_v1_readiness_qwen_64k_batch_profile_requested': self.qwen_64k_batch_profile,
            'api_v1_readiness_qwen_64k_batch_profile_selected': memory_profile.get('batch_profile') if isinstance(memory_profile, dict) else None,
