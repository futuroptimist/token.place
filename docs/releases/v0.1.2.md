...
## Relay metrics implementation slice (Prompt 06)

This implementation adds a bounded, relay-blind Prometheus slice for the
single-pod, one-worker, in-memory relay runtime. It is an application-only
change for the relay deployed by Sugarkube; the canonical Helm chart remains
...
| --- | --- | --- | --- |
| `tokenplace_http_requests_total` | Counter | `method`, `route`, `status_class`, `provider_mode`, `outcome` | Bounded HTTP request totals using normalized routes and status classes. |
| `tokenplace_http_request_duration_seconds` | Histogram | `method`, `route`, `status_class`, `provider_mode`, `outcome` | Coarse relay HTTP latency buckets. |
| `tokenplace_relay_queue_depth` | Gauge | `provider_mode` | Current aggregate API v1 distributed relay queue depth. |
| `tokenplace_relay_oldest_queued_request_age_seconds` | Gauge | none | Age of the oldest queued encrypted API v1 request, or zero when empty. |
| `tokenplace_compute_nodes_registered` | Gauge | none | Registered API v1 compute-node count. |
| `tokenplace_compute_nodes_healthy` | Gauge | none | Healthy API v1 compute-node count using the relay lease/staleness semantics. |
| `tokenplace_compute_node_lease_age_seconds` | Histogram | none | Lease age observations without node identity. |
| `tokenplace_compute_node_evictions_total` | Counter | `reason` | Compute-node removals by fixed reason. |
| `tokenplace_relay_in_flight_requests` | Gauge | none | Current API v1 requests claimed by compute nodes and awaiting response. |
| `tokenplace_relay_oldest_in_flight_age_seconds` | Gauge | none | Age of the oldest in-flight API v1 request, or zero when empty. |
| `tokenplace_relay_request_outcomes_total` | Counter | `outcome` | Terminal API v1 relay outcomes counted once at state transition. |
| `tokenplace_build_info` | Gauge | `version`, `revision` | Public release metadata fixed at `1`. |
| `tokenplace_instrumentation_up` | Gauge | none | Metrics initialization status. |

The existing compatibility counter `tokenplace_relay_requests_total` remains in
place with its historical `method`, `endpoint`, and `status` labels so existing
scrapes and dashboards do not break while operators migrate to the bounded
`tokenplace_http_requests_total` contract.

### Fixed label enums and normalization

`provider_mode` is restricted to `public_api`, `relay_client_read`,
`compute_node_control_plane`, `operational`, and `unknown`. The relay queue-depth
gauge uses the aggregate bounded value `distributed` for API v1 relay work.

`outcome` is restricted to `completed`, `cancelled`, `expired`, `timed_out`,
`rate_limited`, `dependency_failure`, and `failed`. HTTP status labels are status
classes such as `2xx`, `4xx`, and `5xx`. Route labels are normalized relay route
groups, not raw paths or arbitrary URLs.

`tokenplace_compute_node_evictions_total` uses the fixed `reason` enum
`stale_lease` and `explicit_unregister`.

### Metrics authentication

`/metrics` stays exempt from public API rate limits. If
`TOKENPLACE_METRICS_TOKEN` is set, scrapers must send
`Authorization: Bearer <token>`. The relay validates the bearer token with a
constant-time comparison. If the token is unset, `/metrics` preserves the
existing unauthenticated scrape behavior for local and compatibility workflows.

### Privacy invariants

Metrics and touched structured logs remain relay-blind. Metric labels never
include plaintext prompts, plaintext responses, ciphertext, keys, public keys,
request IDs, client IDs, node IDs, IP addresses, auth headers, model names,
arbitrary URLs, user agents, raw errors, or payload-derived sizes. The metrics
slice aggregates node and work state instead of hashing identities into labels.
Gauge callbacks take existing relay locks, do not mutate relay state, and return
zero for oldest-age gauges when no queued or in-flight work exists.

### PromQL examples

```promql
sum(tokenplace_relay_queue_depth{provider_mode="distributed"})
```

```promql
max(tokenplace_relay_oldest_queued_request_age_seconds)
```

```promql
tokenplace_compute_nodes_healthy == 0
  and sum(rate(tokenplace_http_requests_total{route="/api/v1/relay/requests"}[5m])) > 0
```

```promql
histogram_quantile(
  0.95,
  sum by (le, route) (rate(tokenplace_http_request_duration_seconds_bucket[5m]))
)
```

```promql
sum by (outcome) (increase(tokenplace_relay_request_outcomes_total[1h]))
```

This section documents source-level behavior only. It does not mark staging or
production Prometheus, dashboard, alerting, scrape, or Sugarkube release gates as
complete without live cluster evidence.
