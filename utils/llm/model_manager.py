        ...
        if method == 'create_chat_completion_from_rendered_prompt':
            ...
            def _attempt_plain_completion(method_name, attempted_kwargs, call, tokenization_variant_id=''):
                try:
                    ...
                    match = re.search('llama_decode returned\\s+(-?\\d+)', str(exc or ''))
                    if match:
                        try:
                            eval_return_code = int(match.group(1))
                        except ValueError:
                            eval_return_code = None
                    if eval_return_code is not None:
                        plain_capabilities['plain_completion_eval_return_code'] = eval_return_code
                    attempts.append({
                        'method': method_name,
                        'attempted_kwarg_names': ','.join(attempted_kwargs),
...
class ModelManager:
    ...
    def _runtime_init_kwargs(self, llama_cls: Any, n_gpu_layers: int, llama_cpp_module: Any = None, runtime_profile: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if needs_yarn:
            ...
            rope_freq_scale = 1.0 / computed_multiplier if computed_multiplier else 0.0
            configuration_valid = (
