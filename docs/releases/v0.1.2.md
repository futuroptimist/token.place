...
## Implemented relay metrics slice (application baseline)

The relay now exposes a bounded, relay-blind Prometheus metrics slice at `/metrics`
for the single-pod, one-worker, in-memory runtime. This application baseline does
not change the canonical Helm chart and is not evidence that staging or production
...
| --- | --- | --- | --- |
| `tokenplace_http_requests_total` | Counter | `method`, `route`, `status_class`, `provider_mode`, `outcome` | Uses Flask route templates and status classes such as `2xx`, not raw paths. |
| `tokenplace_http_request_duration_seconds` | Histogram | `method`, `route`, `status_class`, `provider_mode`, `outcome` | Uses coarse relay-oriented buckets from milliseconds through 60 seconds. |
| `tokenplace_relay_queue_depth` | Gauge | `provider_mode` | Aggregate encrypted API v1 relay queue depth. |
| `tokenplace_relay_oldest_queued_request_age_seconds` | Gauge | none | Zero when there are no queued requests. |
| `tokenplace_compute_nodes_registered` | Gauge | none | API v1 registered compute-node count. |
| `tokenplace_compute_nodes_healthy` | Gauge | none | Reuses the runtime lease/staleness semantics used for scheduling and eviction. |
| `tokenplace_compute_node_lease_age_seconds` | Histogram | none | Observes aggregate lease age on registration and heartbeat without node identity. |
| `tokenplace_compute_node_evictions_total` | Counter | `reason` | Fixed reasons: `stale_lease`, `server_unregistered`. |
| `tokenplace_relay_in_flight_requests` | Gauge | none | Active API v1 requests claimed by compute nodes and not yet terminal. |
| `tokenplace_relay_oldest_in_flight_age_seconds` | Gauge | none | Zero when there are no in-flight requests. |
| `tokenplace_relay_request_outcomes_total` | Counter | `outcome` | Fixed outcomes: `completed`, `cancelled`, `expired`, `timed_out`, `rate_limited`, `dependency_failure`, `failed`. |
| `tokenplace_build_info` | Gauge | `version`, `revision` | Fixed at `1` for process release identity. |
| `tokenplace_instrumentation_up` | Gauge | none | `1` when metrics initialized; `0` if initialization fails. |
| `tokenplace_relay_requests_total` | Counter | `method`, `endpoint`, `status` | Compatibility counter retained for existing dashboards. |

### Privacy and cardinality invariants

- `/metrics` remains exempt from public API rate limits, but when
  `TOKENPLACE_METRICS_TOKEN` is set it requires `Authorization: Bearer <token>`
  and compares the bearer token in constant time.
- Labels are bounded enums or normalized route/status groups. The metrics layer
  does not label request IDs, client IDs, node IDs, IP addresses, public keys,
  model names, URLs, user agents, auth headers, raw errors, ciphertext, keys, or
  payload-derived sizes.
- Relay-owned state remains ciphertext-only plus safe routing metadata. Metrics
  collection takes existing locks, snapshots state without mutation, and returns
  zero-valued oldest-age gauges when no queued or in-flight work exists.

### PromQL examples

```promql
sum(tokenplace_relay_queue_depth)
max(tokenplace_relay_oldest_queued_request_age_seconds)
sum(rate(tokenplace_relay_request_outcomes_total{outcome="timed_out"}[5m]))
sum(tokenplace_compute_nodes_healthy)
histogram_quantile(0.95, sum(rate(tokenplace_http_request_duration_seconds_bucket[5m])) by (le, route))
```
