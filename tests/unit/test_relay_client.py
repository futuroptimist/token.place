...
def test_api_v1_default_deadline_allows_completion_after_legacy_five_minute_budget(monkeypatch):
    client = _standalone_relay_client()
    now = {'value': 1000.0}
    monkeypatch.setattr(relay_client_module.time, 'monotonic', lambda: now['value'])

    deadline = client._api_v1_initial_deadline_from_metadata({
        'request_deadline_remaining_seconds': 480,
        'request_ttl_seconds': 480,
    })
    now['value'] += 301
