...
The implementation lives in the relay-served landing-page client (`static/chat.js`). For every
outgoing API v1 conversation, it constructs a fresh request-message array by prepending one
...
the deployed behavior belongs to the relay's landing-page static asset, not relay routing, compute
...
The landing-page system-message behavior described above is implemented; the remaining migration
phases are separate from that targeted request-construction change.
