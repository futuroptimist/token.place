...
type UiState =
  "idle" | "starting" | "streaming" | "canceled" | "completed" | "failed";
...
const CONTEXT_PROFILES: Array<{
  ...
  {
    id: "8k-fast",
    displayLabel: "8K Fast",
    totalContextTokens: 8192,
    enabled: true,
  },
  {
    id: "64k-full",
    displayLabel: "64K Full",
    totalContextTokens: 65536,
    enabled: true,
  },
];
...
type PartialDesktopConfig = Omit<
  Partial<DesktopConfig>,
  "model_path" | "relay_base_url" | "relay_base_urls" | "preferred_mode"
> & {
  model_path?: unknown;
...
function safeReadinessDiagnosticValue(
  value: unknown,
...
const PACKAGED_RUNTIME_GUIDANCE =
...
type NormalizedDesktopError = {
  message: string;
  code: string | null;
  disablesPythonBridge: boolean;
};

function normalizeDesktopError(error: unknown): NormalizedDesktopError {
  const raw = error instanceof Error ? error.message : String(error);
  const codeMatch = raw.match(
    /desktop_python_(?:runtime_missing|runtime_invalid|override_invalid|development_dependency_missing)/,
  );
  const rawLooksLikeAppleStub =
    /xcode-select|No developer tools were found|CommandLineTools|Xcode\.app|attempted launcher|stdout|stderr|TOKEN_PLACE_(?:SIDEcar_)?PYTHON/i.test(
      raw,
    );
  const code =
    codeMatch?.[0] ??
    (rawLooksLikeAppleStub ? "desktop_python_runtime_missing" : null);
  if (
    code === "desktop_python_runtime_missing" ||
    code === "desktop_python_runtime_invalid"
  ) {
    return {
      message: PACKAGED_RUNTIME_GUIDANCE,
      code,
      disablesPythonBridge: true,
    };
  }
  if (code === "desktop_python_override_invalid") {
    return {
      message:
        "The configured token.place Python override is invalid. Choose a Python 3.11+ interpreter or unset the override.",
      code,
      disablesPythonBridge: true,
    };
  }
  if (code === "desktop_python_development_dependency_missing") {
    return {
      message:
        "Python 3.11+ is required for this development build. Install Python or set the documented token.place Python override.",
      code,
      disablesPythonBridge: true,
    };
  }
...
function formatErrorMessage(error: unknown): string {
  ...
  return normalized.code
    ? `${normalized.message} Diagnostic code: ${normalized.code}`
    : normalized.message;
}
...
function isStoppedOrIdleOperatorStatus(
  ...
  const relayRuntimeState =
    status.relay_runtime_state?.trim().toLowerCase() || "idle";
  ...
  return (
    ...
    (relayRuntimeState === "stopped" ||
      relayRuntimeState === "idle" ||
      relayRuntimeState === "failed") &&
    (warmLoadState === "stopped" ||
      warmLoadState === "idle" ||
      warmLoadState === "failed")
  );
}
...
function readinessDiagnosticsPayload(
  value: unknown,
...
function readinessDiagnosticsFromEventPayload(
  ...
  const diagnostics: ReadinessDiagnostics = {};
  const nestedDiagnostics = readinessDiagnosticsPayload(
    payload.readiness_diagnostics,
  );
...
function relayStatusesPayload(value: unknown): RelayStatus[] {
  ...
  return value
    ...
    .map((item) => ({
      ...
      relay_runtime_state:
        typeof item.relay_runtime_state === "string"
          ? item.relay_runtime_state
          : null,
      last_error: typeof item.last_error === "string" ? item.last_error : null,
      last_request_id:
        typeof item.last_request_id === "string" ? item.last_request_id : null,
      request_count:
        typeof item.request_count === "number" ? item.request_count : undefined,
    }))
    .filter((item) => item.relay_url.trim());
}

function formatRegisteredLabel(
  status: ComputeNodeStatus,
  fallbackRelayCount: number,
): string {
  const configuredUrls = Array.isArray(status.configured_relay_urls)
  ...
  const configuredCount =
    ...
    1;
  const runtimeState =
    status.relay_runtime_state === "idle"
      ? status.warm_load_state
      : status.relay_runtime_state;
  const ready =
    status.running &&
    (status.warm_load_enabled === false ||
      runtimeState === "ready" ||
      runtimeState === "processing");
  const registeredCount = ready
    ? status.registered_relay_count || (status.registered ? 1 : 0)
    : 0;
...
function normalizeContextTier(contextTier: unknown): ContextTier {
  return contextTier === "64k-full" || contextTier === "8k-fast"
    ? contextTier
...
export function normalizeDesktopConfig(
  ...
  const legacyRelayBaseUrl =
    typeof config.relay_base_url === "string" ? config.relay_base_url : "";
  const relayBaseUrls = normalizeRelayUrls(
...
export function primaryRelayUrl(config: DesktopConfig): string {
  return (
    normalizeRelayUrls(config.relay_base_urls, config.relay_base_url)[0] ||
...
export function updateRelayUrlAtIndex(
  ...
  return {
    ...config,
    relay_base_url: primaryRelayUrl({
      ...config,
      relay_base_urls: relayBaseUrls,
    }),
    relay_base_urls: relayBaseUrls,
...
export function removeRelayUrl(
  config: DesktopConfig,
  index: number,
): DesktopConfig {
  ...
  const relayBaseUrls = config.relay_base_urls.filter(
...
function mergeComputeStatusEvent(
  ...
  const payloadSession =
    typeof payload.operator_session_id === "string"
  ...
  const payloadWorkerGeneration =
    typeof payload.worker_generation === "number"
  ...
  return {
    ...
    registered_relay_urls: isStoppedEvent
      ...
      : Array.isArray(payload.registered_relay_urls)
        ...
        : payload.type === "error"
          ? []
          : stoppedBase.registered_relay_urls,
    active_relay_urls: isStoppedEvent
    ...
    model_path:
      typeof payload.model_path === "string"
        ? payload.model_path
        : prev.model_path,
    relay_runtime_state: isStoppedEvent
    ...
    warm_load_state: isStoppedEvent
    ...
    worker_generation:
      typeof payload.worker_generation === "number"
        ? payload.worker_generation
        : prev.worker_generation,
    worker_restart_count:
      typeof payload.worker_restart_count === "number"
        ? payload.worker_restart_count
        : prev.worker_restart_count,
    worker_alive: isStoppedEvent
    ...
    updated_at_ms:
      typeof payload.updated_at_ms === "number"
        ...
        : prev.updated_at_ms,
    context_tier:
      typeof payload.context_tier === "string"
        ? normalizeContextTier(payload.context_tier)
        : prev.context_tier,
    ...
    readiness_diagnostics:
      payload.type === "started"
        ...
        : (readinessDiagnostics ??
          (shouldClearReadinessDiagnostics
            ? {}
            : stoppedBase.readiness_diagnostics)),
  };
}
...
export function App() {
  ...
  const [pythonBridgeUnavailable, setPythonBridgeUnavailable] = useState(false);
  ...
  const relayRuntimeState =
    ...
    computeStatus.relay_runtime_state !== "idle"
      ? computeStatus.relay_runtime_state
    ...
    const initializeConfigAndArtifact = async () => {
      try {
        ...
        if (JSON.stringify(loadedConfig) !== JSON.stringify(normalizedConfig)) {
          invoke("save_config", { config: normalizedConfig }).catch((e) =>
            setError(formatErrorMessage(e)),
          );
        }
        const nodeStatus = {
          ...defaultComputeStatus,
          ...(await invoke<Partial<ComputeNodeStatus>>(
        ...
        {
          const normalized = normalizeDesktopError(e);
          setPythonBridgeUnavailable(
            (prev) => prev || normalized.disablesPythonBridge,
          );
          setError(
            normalized.code
              ? `${normalized.message} Diagnostic code: ${normalized.code}`
  ...
  useEffect(() => {
    const unlisten = listen<SidecarEvent>("inference_event", (evt) => {
      ...
      if (payload.type === "started") {
        ...
        setStatus("failed");
        {
          const normalized = normalizeDesktopError(
  ...
  const scheduleConfigSave = (next: DesktopConfig) => {
    ...
    saveTimerRef.current = window.setTimeout(() => {
      invoke("save_config", { config: configForSave(next) }).catch((e) =>
        setError(formatErrorMessage(e)),
      );
  ...
  const startComputeNode = async () => {
    try {
      ...
      const optimisticStatus = {
        ...
        sequence: computeStatusRef.current.operator_session_id
          ? Number.MAX_SAFE_INTEGER
          : null,
        active_relay_url: primaryRelayUrl(config),
        configured_relay_urls: normalizeRelayUrls(
          config.relay_base_urls,
          config.relay_base_url,
        ),
        relay_statuses: normalizeRelayUrls(
          config.relay_base_urls,
          config.relay_base_url,
        ).map((relayUrl) => ({
          relay_url: relayUrl,
        ...
        registered_relay_count: 0,
        configured_relay_count: normalizeRelayUrls(
          config.relay_base_urls,
          config.relay_base_url,
        ).length,
        registered_relay_urls: [],
      ...
      await invoke("start_compute_node", {
        request: {
          ...
          relay_base_urls: normalizeRelayUrls(
            config.relay_base_urls,
            config.relay_base_url,
          ),
          mode: config.preferred_mode,
      ...
      {
        const normalized = normalizeDesktopError(message);
        setPythonBridgeUnavailable(
          (prev) => prev || normalized.disablesPythonBridge,
        );
        setError(
    ...
    >
      ...
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <button
          ...
          disabled={isDownloadingModel || pythonBridgeUnavailable}
        >
      ...
      <section aria-labelledby="relay-urls-heading" style={{ marginTop: 12 }}>
        <h2 id="relay-urls-heading" style={{ fontSize: 16, marginBottom: 8 }}>
          Relay URLs
        </h2>
        ...
        {config.relay_base_urls.map((relayUrl, index) => {
          ...
          return (
            ...
            >
              <label htmlFor={inputId} style={{ minWidth: 92 }}>
                Relay URL {index + 1}
              </label>
              <input
                id={inputId}
                ...
                onChange={(e) =>
                  updateConfig(
                    updateRelayUrlAtIndex(config, index, e.target.value),
                  )
                }
              />
              {index > 0 && (
        ...
        <button
          ...
          disabled={
            operatorEditControlsDisabled ||
            config.relay_base_urls.length >= MAX_RELAY_BASE_URLS
          }
          style={{ marginTop: 8 }}
        >
          Add new relay URL
        ...
        <p style={{ marginTop: 6, fontSize: 12, color: "#555" }}>
          Up to {MAX_RELAY_BASE_URLS} relay URLs are supported. Blank entries
          are ignored when saved or started.
        </p>
      </section>
      ...
      <section style={{ marginTop: 14, border: "1px solid #ddd", padding: 12 }}>
        ...
        <label style={{ display: "block", marginBottom: 8 }}>
          ...
          <select
            ...
            onChange={(event) =>
              updateConfig({
                ...config,
                context_tier: normalizeContextTier(event.target.value),
              })
            }
            style={{ display: "block", marginTop: 4 }}
          >
            {CONTEXT_PROFILES.filter((profile) => profile.enabled).map(
              (profile) => (
        ...
        <p style={{ marginTop: 0, fontSize: 12, color: "#555" }}>
          Changing tiers requires Stop Operator followed by Start Operator.
        </p>
        <div style={{ display: "flex", gap: 8 }}>
        ...
        <p style={{ marginBottom: 0 }}>
          Registered:{" "}
          <strong>
            {formatRegisteredLabel(
              computeStatus,
              normalizeRelayUrls(config.relay_base_urls, config.relay_base_url)
                .length,
            )}
          </strong>
        </p>
        <p style={{ marginBottom: 0 }}>
          Relay runtime state: <code>{relayRuntimeState}</code>
        </p>
        <p style={{ marginBottom: 0 }}>
          Context tier:{" "}
          <code>
            {displayStatusValue(
              computeStatus.context_tier,
              config.context_tier,
            )}
          </code>
        </p>
        <p style={{ marginBottom: 0 }}>
          Context window:{" "}
          <code>
            {computeStatus.context_window_tokens ??
              CONTEXT_PROFILES.find(
                (profile) => profile.id === config.context_tier,
              )?.totalContextTokens ??
              8192}
          </code>{" "}
          tokens
        ...
        <p style={{ marginBottom: 0 }}>
          Runtime path:{" "}
          <code>
            {displayStatusValue(computeStatus.runtime_path, "pending")}
          </code>
        </p>
        <p style={{ marginBottom: 0 }}>
          Relay runtime path:{" "}
          <code>
            {displayStatusValue(computeStatus.relay_runtime_path, "pending")}
          </code>
        </p>
        <p style={{ marginBottom: 0 }}>
          Worker state:{" "}
          <strong>
            {displayStatusValue(
              computeStatus.worker_state,
              computeStatus.running ? "starting" : "stopped",
            )}
          </strong>
        </p>
        <p style={{ marginBottom: 0 }}>
          Worker alive:{" "}
          <strong>
            {computeStatus.worker_alive === null
              ? "unknown"
              : computeStatus.worker_alive
                ? "yes"
                : "no"}
          </strong>
        </p>
        <p style={{ marginBottom: 0 }}>
          Worker generation:{" "}
          <code>{computeStatus.worker_generation ?? "unknown"}</code>
        </p>
        <p style={{ marginBottom: 0 }}>
          Worker restart count:{" "}
          <code>{computeStatus.worker_restart_count ?? 0}</code>
        </p>
        <p style={{ marginBottom: 0 }}>
          Last worker error code:{" "}
          <code>{computeStatus.last_worker_error_code || "none"}</code>
        </p>
        <p style={{ marginBottom: 0 }}>
          Last worker exit code:{" "}
          <code>{computeStatus.last_worker_exit_code ?? "none"}</code>
        </p>
        <p style={{ marginBottom: 0 }}>
          Active relay URL:{" "}
          <code>
            {displayStatusValue(
              computeStatus.active_relay_url,
              primaryRelayUrl(config),
            )}
          </code>
        </p>
        <p style={{ marginBottom: 0 }}>
          ...
          <code>
            {(Array.isArray(computeStatus.configured_relay_urls) &&
            computeStatus.configured_relay_urls.length
              ? computeStatus.configured_relay_urls
              : normalizeRelayUrls(
                  config.relay_base_urls,
                  config.relay_base_url,
                )
        ...
        {Array.isArray(computeStatus.relay_statuses) &&
              ...
              <ul style={{ marginTop: 4, paddingLeft: 20 }}>
                {computeStatus.relay_statuses.map((relayStatus) => (
                  <li key={relayStatus.relay_url}>
                    <code>{relayStatus.relay_url}</code> —{" "}
                    {relayStatus.registered ? "registered" : "not registered"}
                    {relayStatus.relay_runtime_state
                      ? ` (${relayStatus.relay_runtime_state})`
                      : ""}
                    {relayStatus.last_request_id
                      ? ` request ${relayStatus.last_request_id}`
                      : ""}
        ...
        <p style={{ marginBottom: 0 }}>
          Effective mode:{" "}
          <code>
            {displayStatusValue(computeStatus.effective_mode, "pending")}
          </code>
        </p>
        <p style={{ marginBottom: 0 }}>
          Backend available:{" "}
          <code>
            {displayStatusValue(computeStatus.backend_available, "pending")}
          </code>
        </p>
        <p style={{ marginBottom: 0 }}>
          Backend selected:{" "}
          <code>
            {displayStatusValue(computeStatus.backend_selected, "pending")}
          </code>
        </p>
        <p style={{ marginBottom: 0 }}>
          Backend used:{" "}
          <code>
            {displayStatusValue(computeStatus.backend_used, "pending")}
          </code>
        </p>
        <p style={{ marginBottom: 0 }}>
          Fallback reason:{" "}
        ...
        <p style={{ marginBottom: 0 }}>
          ...
          <code>
            {formatReadinessDiagnostics(computeStatus.readiness_diagnostics)}
          </code>
        </p>
        <p style={{ marginBottom: 0 }}>
          Model path:{" "}
        ...
        <p style={{ marginBottom: 0 }}>
          Operator debug log:{" "}
          <code>{computeStatus.log_file_path || "not created yet"}</code>
        </p>
        <div
        ...
        >
          <button
            type="button"
            disabled={!computeStatus.log_file_path}
            onClick={refreshOperatorLog}
          >
            Open debug log
          </button>
          <button
        ...
        {isDebugConsoleOpen && (
          <section
            aria-label="Operator debug console"
            style={{ marginTop: 10 }}
          >
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <strong>Operator debug console</strong>
              <button type="button" onClick={refreshOperatorLog}>
                Refresh
              </button>
              <button
                type="button"
                onClick={() => {
                  ...
                  writeText
                    .call(navigator.clipboard, operatorLogText)
