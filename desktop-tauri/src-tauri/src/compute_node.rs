...
fn readiness_diagnostic_chunks(payload: &Value) -> Vec<String> {
    ...
    let mut keys: Vec<String> = diagnostics.keys().cloned().collect();
    keys.sort();
    let mut chunks: Vec<Map<String, Value>> = Vec::new();
    let mut current = Map::new();
    for key in keys {
        let Some(value) = diagnostics.get(&key).cloned() else {
            continue;
        };
        let mut probe = current.clone();
        probe.insert(key.clone(), value.clone());
        let line = readiness_chunk_line(session, sequence, 0, 1, probe.clone());
        if !current.is_empty() && line.len() > 3500 {
            chunks.push(current);
            current = Map::new();
        }
    ...
    chunks
        .into_iter()
        .enumerate()
        .map(|(idx, chunk)| readiness_chunk_line(session, sequence, idx, count, chunk))
        .collect()
}

fn readiness_chunk_line(
    session: &str,
    sequence: u64,
    chunk_index: usize,
    chunk_count: usize,
    ...
    if serialized.len() <= 3500 {
        ...
        serde_json::json!({
            ...
            "chunk_index": chunk_index,
            "chunk_count": chunk_count,
            "diagnostics": {"chunk_omitted": "length_limit"},
        })
        .to_string()
    }
}
...
pub async fn start_compute_node(
    ...
    while let Some(line) = lines.next_line().await? {
        ...
        if let Ok(payload_for_log) = serde_json::from_str::<Value>(&line) {
            for readiness_line in readiness_diagnostic_chunks(&payload_for_log) {
                append_operator_log_line(
                    &log_sink,
