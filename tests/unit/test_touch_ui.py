...
def test_landing_chat_js_avoids_relay_v2_streaming_path():
    ...
    assert "/api/v1/relay/servers/next" in chat_js, (
        "landing chat must select a sticky compute node via API v1 relay next-server route"
    )
    assert "/api/v1/relay/requests" in chat_js, (
        "landing chat must submit ciphertext envelopes through API v1 relay requests"
    )
    assert "/api/v1/relay/responses/retrieve" in chat_js, (
        "landing chat must retrieve ciphertext responses through API v1 relay response retrieval"
    )
    assert "/api/v1/chat/completions" not in chat_js, (
        "landing chat must not use server-side API v1 chat completions selection"
    )
