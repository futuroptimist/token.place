...
def _control_plane_identity_for_request(
    ...
    client_identity = (
        client_identity or ClientIdentityPolicy.from_environment().client_address()
    )
...
def _install_control_plane_rate_limiter(
    app, storage_uri: str | None, client_identity_policy: ClientIdentityPolicy
) -> None:
    ...
    def _enforce_control_plane_rate_limit():
        ...
        remote_address = client_identity_policy.limiter_key()
        checks: list[tuple[str, str, Any]] = [
...
def init_app(app, *, metrics_registry=None, metrics_export_defaults=True, metrics_path="/metrics"):
    ...
    # Flask-Limiter's INFO rejection message includes its computed key. The
    # response and our bounded telemetry are sufficient; never emit that key.
    ...
    limiter_key_func = client_identity_policy.limiter_key
    ...
    _install_control_plane_rate_limiter(app, limiter_storage_uri, client_identity_policy)
