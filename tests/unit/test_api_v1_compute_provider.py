...
def test_distributed_compute_provider_fails_closed_with_structured_error():
...
@pytest.mark.parametrize(
    ("fallback_enabled", "expected_backend_path", "expected_status"),
    [
        ("1", "fallback_local_in_process", 200),
        ("0", None, 503),
    ],
)
def test_api_v1_chat_completion_reports_backend_path_when_distributed_mode_selected(
    monkeypatch, fallback_enabled, expected_backend_path, expected_status
):
    monkeypatch.setattr(
        compute_provider,
        "generate_response",
    ...
    monkeypatch.setenv("TOKENPLACE_API_V1_COMPUTE_PROVIDER", "distributed")
    monkeypatch.setenv("TOKENPLACE_DISTRIBUTED_COMPUTE_URL", "https://node-a.example")
    monkeypatch.setenv("TOKENPLACE_API_V1_DISTRIBUTED_FALLBACK", fallback_enabled)
    compute_provider._build_api_v1_compute_provider.cache_clear()
    ...
    try:
        ...
        if expected_backend_path is None:
            assert "X-Tokenplace-API-V1-Execution-Backend-Path" not in response.headers
        else:
            assert (
...
def test_api_v1_chat_completion_returns_structured_error_when_distributed_no_fallback(monkeypatch):
    monkeypatch.setenv("TOKENPLACE_API_V1_COMPUTE_PROVIDER", "distributed")
    monkeypatch.setenv("TOKENPLACE_DISTRIBUTED_COMPUTE_URL", "https://node-a.example")
    monkeypatch.setenv("TOKENPLACE_API_V1_DISTRIBUTED_FALLBACK", "0")
    compute_provider._build_api_v1_compute_provider.cache_clear()
