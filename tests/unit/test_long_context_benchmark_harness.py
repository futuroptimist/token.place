...
def test_owned_runner_posix_fails_closed_when_group_cleanup_is_unavailable(monkeypatch):
...
if hasattr(os, 'killpg'):
    del os.killpg
sys.argv = ['scripts/long_context_benchmark.py', 'matrix-plan']
runpy.run_path(sys.argv[0], run_name='__main__')
"""
    completed = subprocess.run([sys.executable, "-c", script], text=True,
        capture_output=True, check=False)
    assert completed.returncode == 0, completed.stderr
    assert json.loads(completed.stdout) == h.build_matrix_plan()
