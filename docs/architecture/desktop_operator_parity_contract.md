...
4. **Relay registration eligibility**: `registered: true` is allowed only when the operator is running, the relay runtime is `ready` or `processing`, warm-load has completed, and at least one API v1 relay registration is fresh for a configured relay URL. Multi-relay status must report configured and registered counts (for example prod+staging `2/2`) without exposing full public keys. Missing desktop runtime dependencies, failed runtime setup, and `probe_only`/pending bootstrap states are not registration-eligible; `registered` stays `false` until the relay-processing runtime proves readiness.
...
6. **Stop operator**: Stop cancels polling, unregisters from every configured relay when possible, stops the relay clients, emits `stopped`, and reports `registered: false`. Registrations that cannot be explicitly removed must expire from relay diagnostics.
...
10. **Error/fallback behavior**: relay-owned logs/state remain ciphertext-only plus safe routing metadata. Any runtime or dependency failure must fail closed with diagnostics that explain platform, action, interpreter/import root when relevant, and a next step. A per-relay registration or polling failure is isolated from other configured relays; healthy relays continue to poll and the UI status reflects the reduced registered count.

## Validation checklist

The release and development checklist that keeps this contract evergreen lives in [Desktop parity validation checklist](../desktop_parity_validation.md). Use that shared checklist for Windows CUDA, macOS Metal, CPU fallback, packaged resource resolution, warm-load, API v1 E2EE relay registration/chat, stopped-only relay URL editing, prod+staging multi-relay registration (`https://token.place` plus `https://staging.token.place`), partial relay failure isolation, Stop, Start after Stop, and two-node round-robin release evidence.
