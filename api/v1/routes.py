...
def _handle_chat_completion_request(data):
    ...
    try:
        ...
        if isinstance(completion_result, ApiV1CompletionResult):
            assistant_message = completion_result.message
            backend_finish_reason = completion_result.finish_reason
            authoritative_usage = completion_result.usage
        else:
            # Explicit rolling-upgrade/test-provider compatibility path.
        ...
        finish_reason = backend_finish_reason or ("tool_calls" if tool_calls else "stop")
