...
def test_matrix_plan_entry_point_imports_without_os_killpg():
    ...
    script = repository / "scripts" / "long_context_benchmark.py"
    ...
    completed = subprocess.run(
        [sys.executable, "-c", probe], cwd=repository, check=False,
...
def test_owned_runner_windows_cleans_exact_pid_and_reaps(cleanup_outcome, monkeypatch):
    ...
    with pytest.raises(subprocess.TimeoutExpired):
        h._run_owned_runner(["runner"], 1, 2,
            ...
            killpg=lambda *_: pytest.fail("Windows cleanup resolved POSIX killpg"),
            platform_name="nt")
...
def test_owned_runner_posix_fails_closed_when_killpg_is_unavailable(monkeypatch):
    process = _TimedOutProcess(["timeout"])
    process.stdout = type("Output", (), {"read": lambda self, size: b""})()
    monkeypatch.delattr(h.os, "killpg", raising=False)
