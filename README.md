...
|-----------------|--------------|--------------------------------------------------------------------|
| API_RATE_LIMIT  | 60/hour      | Per-IP rate limit for user-facing API requests; API v1 compute-node control-plane routes use separate budgets. Public UI and release-metadata `GET`/`HEAD` requests (`/`, `/api/v1/meta`, and `/api/v1/version`) do not consume this quota |
...
| TOKENPLACE_RATE_LIMIT_TRUSTED_PROXIES | (empty) | Comma-separated canonical immediate-proxy networks allowed to supply the single `CF-Connecting-IP` rate-limit identity; invalid or dangerously broad values fail startup. See [the trust-boundary contract](docs/ops/trusted-proxy-rate-limit-identity.md) |
