...
function normalizeQwen64kBatchProfile(value: unknown): Qwen64kBatchProfile {
  return value === 'safe' || value === 'balanced' || value === 'experimental'
    ? value : DEFAULT_QWEN_64K_BATCH_PROFILE;
}
    ...
    <main style={{ maxWidth: 820, margin: '20px auto', fontFamily: 'sans-serif' }}>
      ...
      <section style={{ marginTop: 14, border: '1px solid #ddd', padding: 12 }}>
        ...
        <p id="qwen-64k-batch-help" style={{ marginTop: 0, fontSize: 12, color: '#555' }}>
          64K only; stop and restart the operator after changing. Balanced: 512/256. Safe: 256/128. Experimental: 1024/512 and can increase memory use or instability.
          {config.context_tier !== '64k-full' ? ' Inactive for the 8K tier; the saved preference is not applied.' : ''}
        </p>
        <div style={{ display: 'flex', gap: 8 }}>
