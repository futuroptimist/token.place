...
# 8a. Always run the focused landing-page Markdown regression.
run_test \
    "Relay Landing Page Markdown Rendering" \
    "$PYTHON_CMD -m pytest tests/e2e/test_ui.py::test_markdown_rendering_stream_updates -v -x $COVERAGE_ARGS" \
    "Verifying user and assistant fenced Markdown renders as escaped code blocks"
