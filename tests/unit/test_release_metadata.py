...
def test_staging_prefers_immutable_image_tag_for_display(monkeypatch):
    _clear_metadata_env(monkeypatch)
    monkeypatch.setenv("TOKENPLACE_DEPLOY_ENV", "staging")
    monkeypatch.setenv("TOKENPLACE_RELEASE_VERSION", "0.1.1")
...
def test_staging_semver_image_tag_with_git_sha_displays_git_ref(monkeypatch):
    _clear_metadata_env(monkeypatch)
    monkeypatch.setenv("TOKENPLACE_DEPLOY_ENV", "staging")
    monkeypatch.setenv("TOKENPLACE_RELEASE_VERSION", "v0.1.1")
...
def test_prod_prefers_release_version_over_image_tag_for_display(monkeypatch):
    _clear_metadata_env(monkeypatch)
    monkeypatch.setenv("TOKENPLACE_DEPLOY_ENV", "prod")
    monkeypatch.setenv("TOKENPLACE_RELEASE_VERSION", "0.1.1")
    monkeypatch.setenv("TOKENPLACE_IMAGE_TAG", "main-830d0a4")
...
def test_prod_without_release_version_uses_semver_image_tag(monkeypatch, tmp_path):
    _clear_metadata_env(monkeypatch)
    monkeypatch.setattr(release_metadata, "_REPO_ROOT", tmp_path)
    release_metadata._read_chart_app_version.cache_clear()
    release_metadata._read_package_version.cache_clear()
...
def test_deployed_like_image_tag_does_not_fall_back_to_dev(monkeypatch, tmp_path):
    _clear_metadata_env(monkeypatch)
    monkeypatch.setattr(release_metadata, "_REPO_ROOT", tmp_path)
    release_metadata._read_chart_app_version.cache_clear()
    release_metadata._read_package_version.cache_clear()
...
def test_asset_version_distinguishes_different_sha256_digests(monkeypatch):
    _clear_metadata_env(monkeypatch)
    monkeypatch.setenv("TOKENPLACE_RELEASE_VERSION", "0.1.1")
    monkeypatch.setenv("TOKENPLACE_IMAGE_TAG", "sha256:" + "0" * 63 + "1")
    first = release_metadata.resolve_asset_version("staging.token.place")

    monkeypatch.setenv("TOKENPLACE_IMAGE_TAG", "sha256:" + "0" * 63 + "2")
    second = release_metadata.resolve_asset_version("staging.token.place")

    assert first == "sha256-" + "0" * 63 + "1"
    assert second == "sha256-" + "0" * 63 + "2"
    assert first != second
