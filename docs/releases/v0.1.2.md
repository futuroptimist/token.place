...
or tests. When `serviceMonitor.enabled=true`, the chart renders a
`monitoring.coreos.com/v1` `ServiceMonitor` selecting the canonical token.place Service
by its `app.kubernetes.io/name` and `app.kubernetes.io/instance` labels, scraping the
named `http` port at `metrics.path` (default `/metrics`).
...
making the selector irreversible. Bounded target relabeling hooks are limited to fixed
`app`, `environment`, `release`, and `cluster` labels.
...
helm template tokenplace charts/tokenplace --namespace tokenplace \
  ...
  --set serviceMonitor.targetLabels.release=main-REPLACE_SHORTSHA \
