...
def test_metrics_mode_renders_reviewed_startup_value(mode: str) -> None:
...
@pytest.mark.parametrize("mode", ("", "NORMAL", "Degraded", "unknown"))
def test_metrics_mode_schema_rejects_invalid_values(mode: str) -> None:
    result = _helm_template("--set-string", f"metrics.mode={mode}", check=False)
