...
from inference_timeout import DEFAULT_API_V1_OUTER_TIMEOUT_SECONDS
...
class DistributedApiV1ComputeProvider:
    ...
    base_url: str
    timeout_seconds: float = DEFAULT_API_V1_OUTER_TIMEOUT_SECONDS
...
def _build_api_v1_compute_provider(
    ...
    timeout_raw = os.environ.get(
        ...
        str(DEFAULT_API_V1_OUTER_TIMEOUT_SECONDS),
    )
    try:
        timeout_seconds = max(float(timeout_raw), 1.0)
    except (TypeError, ValueError):
        timeout_seconds = DEFAULT_API_V1_OUTER_TIMEOUT_SECONDS
