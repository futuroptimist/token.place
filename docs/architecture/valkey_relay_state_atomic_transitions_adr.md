...
`_schema:v1` sentinel is created in the registration transition, so a missing set is
...
bounded `node_transitions` index.  Completion is retained independently in a short
