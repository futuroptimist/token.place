...
try:
    ...
    try:
        ...
        constructor_kwarg_support = {name: bool(name in params or constructor_has_var_kwargs) for name in CONSTRUCTOR_CAPABILITY_KWARGS}
    except (TypeError, ValueError):
        constructor_kwarg_support = {name: False for name in CONSTRUCTOR_CAPABILITY_KWARGS}
        ...
        for owner in (llama_cpp, getattr(llama_cpp, "llama_cpp", None)):
            ...
            for name in names:
                ...
                if isinstance(value, int) and not isinstance(value, bool):
                    return value
        return None
    q8_kv_cache_type_value = _const_value("GGML_TYPE_Q8_0", "LLAMA_TYPE_Q8_0")
    q4_kv_cache_type_value = _const_value("GGML_TYPE_Q4_0", "LLAMA_TYPE_Q4_0")
    f16_kv_cache_type_value = _const_value("GGML_TYPE_F16", "LLAMA_TYPE_F16")
    ...
    qwen_64k_yarn_support = "supported" if yarn_rope_supported else ("unknown" if not constructor_signature_inspectable else "unsupported")
