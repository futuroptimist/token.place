...
def relay_api_v1_chat_completions():
    """Fail closed for plaintext API v1 relay chat-completion dispatch."""
    ...
    return jsonify(
        {
            'error': {
                ...
                'message': 'Distributed relay API v1 is disabled until end-to-end encryption is restored.',
            }
        }
    ), 503
...
def relay_api_v1_source():
    """Fail closed for plaintext API v1 relay source submission."""
    ...
    if queued_requests:
        ...
        while queued_requests and len(batch) < max_batch_size:
            ...
            if 'api_v1_request' in request_payload:
                logging.warning(
                    "Dropping plaintext api_v1_request relay queue item for server=%s",
                    public_key,
                )
                continue
