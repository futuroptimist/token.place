...
or tests. When chart metrics are disabled, the Deployment sets
`TOKENPLACE_METRICS_DISABLED=1` so `/metrics` fails closed instead of remaining
public through the catch-all app Ingress. When `serviceMonitor.enabled=true`, the
...
making the selector irreversible; additional labels may not override chart-owned
metadata keys. Bounded metric relabeling hooks are configured with
`serviceMonitor.metricLabels` and limited to fixed `app`, `environment`, `release`,
and `cluster` labels.

This chart slice intentionally does not add a public `/metrics` Ingress path, does not
copy the legacy `deploy/charts/tokenplace-relay` monitoring resources wholesale, and does
not alter accepted relay architecture constraints: one replica, one Gunicorn worker,
`Recreate` rollout, and in-memory relay state remain the canonical Sugarkube runtime
...
helm template tokenplace charts/tokenplace --namespace tokenplace \
  ...
  --set serviceMonitor.metricLabels.environment=staging \
  --set serviceMonitor.metricLabels.release=main-REPLACE_SHORTSHA \
  --set serviceMonitor.metricLabels.cluster=sugarkube-staging
helm lint charts/tokenplace
```

After deployment, prove internal scrape success, public denial, release discovery, and
bounded target labels with the commands in the README's optional internal Prometheus
scrape section.
