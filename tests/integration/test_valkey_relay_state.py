...
def test_selection_final_capacity_race_is_atomic_across_valkey_stores(valkey_server):
    ...
    try:
        ...
        assert (
            sum(isinstance(outcome, RelayStateNoCapacity) for outcome in outcomes) == 1
        )
        ...
        assert (
            sum(isinstance(item, RelayStateNoCapacity) for item in memory_outcomes) == 1
        )
    finally:
        keys = [
            cfg.key("schema"),
...
def test_selection_once_only_same_identity_across_valkey_stores(valkey_server):
    ...
    try:
        first.register("node", _capabilities(concurrency=1), _digest("owner"))
        deadline = first._foundation.server_time()[0] + 60
        outcomes = _race_select_and_reserve((first, second), ("same", "same"), deadline)
        ...
        assert (
            sum(outcome.reservation_token is not None for outcome in memory_outcomes)
            == 1
        )
...
def test_scheduler_fairness_cursor_changes_only_for_new_reservations(valkey_server):
    ...
    try:
        ...
        with pytest.raises(RelayStateNoCapacity):
            second.select_and_reserve(
                "client", "failed", "unsupported", "8k-fast", deadline
            )
        ...
        # Deadline authority is retained until the terminal CAS records expiry.
        assert first._foundation._client.exists(reservation_key)
    finally:
        _delete_scheduler_policy_state(
            first,
            ("node-b", "node-a"),
...
def test_cursor_fingerprint_index_preserves_additive_fields_and_evicts_deterministically(
    ...
    try:
        ...
        metadata = {
            "_count": 2,
            "_activity": 2,
            "_fp:1": fingerprints["alpha"],
            "_fp:2": fingerprints["beta"],
            **additive,
        }
        ...
        token = (
            _digest(selected.reservation_token)
            if selected and selected.reservation_token
            else _digest("unused")
        )
...
def test_missing_indexed_lifecycle_fails_closed_without_mutation(valkey_server):
    ...
    try:
        store.register("node", _capabilities(), owner)
        store._foundation._client.zadd(
            cfg.key("requests:deadline"), {missing: deadline}
        )
...
def test_expired_addressed_reservation_is_preserved_for_terminalization(
    valkey_server,
):
    store = _registration_store(
        valkey_server,
    ...
    try:
        ...
        assert store._foundation._client.zcard(cfg.key("requests:deadline")) == 3
        assert store._foundation._client.zcard(cfg.key("reservations:expiry")) == 3
        assert store._foundation._client.hgetall(cfg.key("cursor")) == cursor_before
    finally:
        keys = [
            cfg.key("schema"),
...
def test_cancellation_creates_authenticated_acknowledgeable_control(valkey_server):
    store = _registration_store(valkey_server, uuid.uuid4().hex)
    node_id, identity, owner = (
        "cancel-node",
    ...
    try:
        ...
        result = store.cancel_or_expire_request(*identity, "cancel")
        assert (result.state, result.reason, result.new_outcome) == (
            "cancelled",
            "requester_cancelled",
            True,
        )
        retry = store.cancel_or_expire_request(*identity, "cancel")
        assert not retry.new_outcome
        control = store.renew_claim_or_read_control(
            node_id, owner, "consumer", *identity, claim.generation
        )
        assert (control.state, control.reason, control.acknowledged) == (
            "cancelled",
            "requester_cancelled",
            False,
        )
        ...
        store._foundation._client.delete(
            store._foundation.config.key("control", node, client, request),
            store._foundation.config.key("control:expiry"),
        )
        store.close()
...
def test_response_retrieval_preserves_additive_fields_and_bounded_backlog(
    ...
    try:
        ...
        assert (
            datastore.zrange(keys[10], 0, -1, withscores=True) == terminal_index_before
        )
...
def test_response_retrievals_are_concurrent_replayable_and_independently_derived(
    ...
    try:
        ...
        expected = hmac.new(_ACKNOWLEDGEMENT_KEY, message, hashlib.sha256).hexdigest()
...
def test_response_retrieval_rejects_noncanonical_authority_without_disclosure_or_mutation(
    ...
    fixture = _accepted_retrieval_fixture(
        (store,), f"invalid-{kind}-{uuid.uuid4().hex}"
    )
    node, identity, credential, envelope, _, keys, _ = fixture
    try:
        before = _exact_key_snapshot(store, keys)
        ...
        assert _exact_key_snapshot(store, keys) == before
        assert (
            store.retrieve_encrypted_response(*identity, credential).envelope
            == envelope
        )
    finally:
        _delete_claim_fixture_state(store, (node,), (identity,))
...
def test_response_concurrent_acknowledgements_have_coherent_terminal(
    ...
    try:
        ...
        assert (
            stores[0].retrieve_encrypted_response(*identity, credential).state
            == "acknowledged"
        )
...
def test_response_concurrent_conflicting_acknowledgement_has_one_valid_winner(
    ...
    unrelated_ready = stores[1].retrieve_encrypted_response(*unrelated[1], unrelated[2])
    ...
    try:
        ...
        assert (
            stores[1].retrieve_encrypted_response(*unrelated[1], unrelated[2]).envelope
            == unrelated[3]
        )
    finally:
        for store, original in zip(stores, originals):
...
def test_response_retrieval_cross_identity_credentials_and_tokens_are_fixed_and_non_mutating(
    ...
    try:
        ...
        assert (
            store.retrieve_encrypted_response(*first[1], second[2]).state
        ...
        assert [_exact_key_snapshot(store, item[5]) for item in fixtures] == snapshots
        assert (
            store.retrieve_encrypted_response(*first[1], first[2]).state
            == "response_ready"
        )
    finally:
        _delete_claim_fixture_state(
            store,
            tuple(item[0] for item in fixtures),
            tuple(item[1] for item in fixtures),
        )
        store.close()
...
def test_encrypted_response_retrieval_is_replayable_shared_and_acknowledged_once(
    ...
    try:
        ...
        assert (
            first.retrieve_encrypted_response(*identity, "0" * 64).state
            == "invalid_retrieval_credential"
        )
        assert (
            first.retrieve_encrypted_response(
        ...
        try:
            assert (
                rotated.retrieve_encrypted_response(
                *identity,
                selection.reservation_token,
                initial.acknowledgement_token,
                ).state
...
def test_due_response_retrieval_validates_all_authority_before_cleanup(
    ...
    try:
        ...
        now = sum(
            value / divisor
            for value, divisor in zip(store._foundation.server_time(), (1, 1_000_000))
        )
        ...
        datastore.zadd(
            keys[10], {member: float(expiry_raw), unrelated_member: now + 99}
        )
        if retrieval_state == "response_ready":
            ...
            datastore.zadd(
                keys[8], {member: float(replay_raw), unrelated_member: now + 98}
            )
        else:
...
def test_response_retrieval_rejects_coordinated_noncanonical_timestamp_authority(
    ...
    store = _registration_store(valkey_server, uuid.uuid4().hex)
    fixture = _accepted_retrieval_fixture((store,), f"canonical-authority-{field}")
    node, identity, credential, _, _, keys, member = fixture
    datastore = store._foundation._client
...
def test_encrypted_response_namespace_isolation(valkey_server):
    ...
    try:
        ...
        assert (
            stores[1]._foundation._client.exists(
            authorities[1][0][7], authorities[1][0][9]
            )
            == 0
        )
        assert (
            stores[1]._foundation._client.zscore(
...
def test_encrypted_response_backend_failures_are_typed_redacted_and_not_replayed(
    ...
    try:
        ...
        caplog.clear()
        with caplog.at_level(logging.DEBUG):
            with pytest.raises(expected_error, match=f"^{expected_message}$") as caught:
                store.accept_encrypted_response(
                    node, owner, consumer, *identity, claim.generation, envelope
                )
        ...
        assert all(
            ...
            not in (value if isinstance(value, bytes) else str(value).encode())
            for value in dispatched_args[2:]
        )
        assert _exact_key_snapshot(store, keys) == before
        assert store._foundation._client.exists(keys[7], keys[9]) == 0
...
def test_encrypted_response_concurrent_identical_acceptance_has_one_outcome(
    ...
    try:
        ...
        assert dataclasses.replace(
            results[0], new_outcome=False
        ) == dataclasses.replace(results[1], new_outcome=False)
        keys, member = _response_acceptance_authority(first, node, identity)
...
def test_encrypted_response_retries_retained_retrieval_expired_terminal(valkey_server):
    ...
    try:
        ...
        assert (
            datastore.hget(cfg.key("terminal", client, request), "retrieval_state")
            == b"retrieval_expired"
        )
...
@pytest.mark.parametrize(
    "terminal_fields", ({"additive": "value"}, {"outcome": "completed"})
)
...
@pytest.mark.parametrize(
    "corruption",
    ("empty_node", "oversized_node", "negative_accepted", "future_accepted"),
)
def test_encrypted_response_rejects_invalid_retained_terminal_bounds_without_mutation(
    ...
    store = _registration_store(
        valkey_server,
        namespace,
        response_replay_ttl_seconds=(
            0.05 if retrieval_state == "retrieval_expired" else 300
        ),
            ...
            if corruption.startswith("oversized_"):
                bound = (
                    ...
                    if field == "envelope"
                        ...
                        store.config.max_node_id_bytes
                    if field == "node_id"
...
def test_encrypted_response_cleanup_validates_batch_before_exact_reaping(valkey_server):
    ...
    try:
        ...
        for identity in identities:
            ...
            lifecycle = cfg.key("request", client, request)
            datastore.hset(
                response, "replay_expires_at_epoch", repr(accepted.accepted_at_epoch)
            )
            datastore.hset(
                terminal, "replay_expires_at_epoch", repr(accepted.accepted_at_epoch)
            )
            datastore.zadd(
                cfg.key("responses:expiry"), {member: accepted.accepted_at_epoch}
            )
            records.append(
                (member, response, terminal, lifecycle, accepted.accepted_at_epoch)
            )
        ...
        def snapshot():
            return (
                ...
                tuple(
                    datastore.zrange(key, 0, -1, withscores=True) for key in index_keys
                ),
            )
...
def test_encrypted_response_cleanup_reaps_exact_response_then_terminal(valkey_server):
    ...
    try:
        ...
        datastore.hset(
            terminal,
            mapping={"replay_expires_at_epoch": repr(due), "additive": "preserved"},
        )
...
def test_encrypted_response_inclusive_expiry_boundaries_are_non_mutating(
    ...
    namespace = uuid.uuid4().hex
    first = _registration_store(valkey_server, namespace, claim_ttl_seconds=claim_ttl)
    second = _registration_store(valkey_server, namespace, claim_ttl_seconds=claim_ttl)
...
def test_encrypted_response_first_transition_removes_exact_authority_once(
    ...
    identities = (
        ("transition-client-a", "request-a"),
        ("transition-client-b", "request-b"),
    )
    envelope = EncryptedResponseEnvelope(
        "tokenplace_api_v1_relay_e2ee", 1, "ciphertext", "key", "iv"
    )
    ...
    try:
        ...
        assert (
            tuple(datastore.hgetall(neighbor_keys[index]) for index in (3, 4, 11))
...
def test_claim_queued_request_rejects_malformed_addressed_authority_without_mutation(
    ...
    def snapshot():
        return (
            ...
            tuple(
                client.zrange(key, 0, -1, withscores=True) for key in sorted_set_keys
            ),
        )
...
def test_claim_lease_is_bounded_by_request_deadline(valkey_server):
    ...
    try:
        ...
        assert (
            store._foundation._client.hget(
...
def test_renew_claim_preserves_exact_deadline_representation(valkey_server):
    ...
    try:
        ...
        assert (
            store._foundation._client.hget(claim_key, "lease_expires")
            == stored_deadline
        )
        ...
        assert (
            store._foundation._client.zscore(cfg.key("claims:expiry"), member)
            == decoded_deadline
        )
        ...
        assert store._foundation._client.hgetall(cfg.key("cursor")) == cursor_before
        assert (
            store._foundation._client.xrange(
            cfg.key("queue", store._node_digest(node_id))
            )
            == queue_before
        )
...
def test_completed_record_reads_hide_expired_backlog_beyond_cleanup_batch(
    valkey_server,
):
    namespace = uuid.uuid4().hex
    store = _registration_store(
    ...
    identities = tuple(
        ("expiry-client", f"expiry-request-{index}") for index in range(3)
    )
    response = EncryptedResponseEnvelope(
        "tokenplace_api_v1_relay_e2ee", 1, "ciphertext", "key", "iv"
    )
