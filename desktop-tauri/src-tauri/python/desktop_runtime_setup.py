...
def _normalized_machine(value: Optional[str] = None) -> str:
    machine = (value or platform_module.machine() or "").strip().lower()
    return {
        "amd64": "x86_64",
        "x64": "x86_64",
        "arm64": "aarch64",
...
def _runtime_bootstrap_policy(
    *,
    ...
    if platform_name.startswith("win"):
        if arch == "x86_64":
            return RuntimeBootstrapPolicy(
                ...
                reason="Windows x86_64 CUDA runtime bootstrap is supported",
            )
        return RuntimeBootstrapPolicy(
            platform=platform_name,
            arch=arch,
            ...
            reason=f"CUDA bootstrap is only enabled for Windows x86_64 (detected arch={arch or 'unknown'})",
        )
    ...
    if platform_name == "darwin":
        if arch in {"x86_64", "aarch64"}:
            return RuntimeBootstrapPolicy(
                ...
                backend="metal",
                bootstrap_supported=True,
                reason="macOS Metal runtime bootstrap is supported",
            )
        return RuntimeBootstrapPolicy(
            platform=platform_name,
            arch=arch,
            backend="metal",
            bootstrap_supported=False,
            reason=f"Metal bootstrap is only enabled for macOS arm64/x86_64 (detected arch={arch or 'unknown'})",
        )
    ...
    return RuntimeBootstrapPolicy(
        ...
        backend=None,
        bootstrap_supported=False,
        reason=f"desktop runtime bootstrap is probe-only on platform={platform_name} arch={arch or 'unknown'}",
    )
...
def _runtime_action_for_cpu_fallback(policy: RuntimeBootstrapPolicy) -> str:
    if policy.backend == "metal":
        return "metal_cpu_fallback"
    return "installed_cpu_fallback"


def _runtime_action_for_install_failure(policy: RuntimeBootstrapPolicy) -> str:
    if policy.backend == "metal":
...
SOURCE_BUILD_TIMEOUT_ENV = "TOKEN_PLACE_DESKTOP_RUNTIME_SOURCE_BUILD_TIMEOUT_SECONDS"
...
def _source_build_timeout_seconds() -> int:
    configured = os.getenv(SOURCE_BUILD_TIMEOUT_ENV, "").strip()
    if not configured:
    ...
    try:
        ...
        return PIP_SOURCE_BUILD_TIMEOUT_SECONDS
    return timeout if timeout > 0 else PIP_SOURCE_BUILD_TIMEOUT_SECONDS


def _unversioned_source_plan(backend: str, platform_name: str) -> LlamaCppInstallPlan:
    cmake_args = None
    if backend == "cuda":
    ...
    return LlamaCppInstallPlan(
        ...
        force_cmake=backend in {"cuda", "metal"},
...
def _source_repair_plan(
    backend: str, requirements_path: Path, platform_name: str
) -> LlamaCppInstallPlan:
    try:
        ...
        plans = _fallback_unpinned_plans(platform_name)

    for plan in plans:
        if plan.backend == backend and plan.no_binary:
            return plan

    try:
        plan = llama_cpp_install_plan(platform=platform_name, requirements_path=requirements_path)
    except (FileNotFoundError, OSError, ValueError):
        fallback_plans = _fallback_unpinned_plans(platform_name)
        plan = fallback_plans[0] if fallback_plans else _unversioned_source_plan(backend, platform_name)
...
def _source_repair(
    ...
    *,
    ...
    platform_name = (platform_name or sys.platform).lower()
    plan = _source_repair_plan(backend, requirements_path, platform_name)
    env = os.environ.copy()
    ...
    ok, output = _run_pip_install(cmd, env, timeout_seconds=_source_build_timeout_seconds())
...
def _windows_cuda_source_repair(requirements_path: Path) -> tuple[bool, str]:
    return _source_repair("cuda", requirements_path, platform_name="win32")
...
def desktop_gpu_runtime_failure_message(mode: str, runtime_setup: Dict[str, str]) -> str | None:
    ...
    if policy.backend not in {"cuda", "metal"}:
        return None
    ...
    if policy.backend == "metal":
        fatal_action = selected_mode == "gpu" and (
            runtime_action in GPU_RUNTIME_FATAL_ACTIONS or runtime_action.endswith("_probe_only")
        )
    else:
        fatal_action = runtime_action in GPU_RUNTIME_FATAL_ACTIONS

    if not fatal_action:
        return None

    reason = runtime_setup.get("fallback_reason") or "unknown runtime bootstrap failure"
    platform_label = "macOS Metal" if policy.backend == "metal" else "desktop Windows"
    prerequisite_label = "Metal runtime prerequisites" if policy.backend == "metal" else "CUDA runtime prerequisites"
    return (
        f"GPU provisioning failed for {platform_label} launch "
        f"(mode={selected_mode}, action={runtime_action}): {reason}. "
        f"Verify {prerequisite_label} and llama-cpp-python {policy.backend.upper()} build support."
    )
...
def ensure_desktop_llama_runtime(mode: str, *, repo_root: Optional[Path] = None) -> Dict[str, str]:
    ...
    if not policy.bootstrap_supported:
        ...
        return {
            "selected_backend": "cpu",
            "fallback_reason": (
                f"{policy.reason}; runtime probe found backend={before.backend} "
                f"error={before.error or 'none'}; interpreter={before.interpreter}; "
                f"import_root={target_root}"
            ),
    ...
    if os.getenv(DISABLE_BOOTSTRAP_ENV) == "1":
        return {
            ...
            "fallback_reason": (
                ...
                f"backend={policy.backend}; interpreter={before.interpreter}; import_root={target_root}"
            ),
            "runtime_action": "metal_probe_only" if policy.backend == "metal" else "probe_only",
            **_probe_result_payload(before),
        }
    ...
    if os.getenv(ENABLE_BOOTSTRAP_ENV) != "1":
        return {
            ...
            "fallback_reason": (
                ...
                f"{ENABLE_BOOTSTRAP_ENV}=1 to allow {policy.backend} runtime repair/install; "
                f"interpreter={before.interpreter}; import_root={target_root}"
            ),
            "runtime_action": "metal_probe_only" if policy.backend == "metal" else "probe_only",
            **_probe_result_payload(before),
    ...
    if should_repair and policy.backend:
        if policy.backend == "cuda" and policy.platform.startswith("win"):
            source_ok, source_log = _windows_cuda_source_repair(requirements_path)
        else:
            source_ok, source_log = _source_repair(
                policy.backend,
                requirements_path,
        ...
        if source_ok:
            ...
            if after.gpu_offload_supported and after.backend == policy.backend:
                return {
                    ...
                    "runtime_action": _runtime_action_for_installed_backend(policy.backend),
            ...
            last_error = (
                ...
                f"check {policy.backend.upper()} toolkit/build prerequisites"
            )
    ...
    for plan in plans:
        ...
        timeout_seconds = (
            _source_build_timeout_seconds() if plan.no_binary else PIP_INSTALL_TIMEOUT_SECONDS
        )
        ...
        if plan.backend == "cpu":
            return {
                "selected_backend": "cpu",
                "fallback_reason": (
                    f"{policy.backend.upper() if policy.backend else 'GPU'} runtime unavailable after repair; "
                    "using CPU runtime"
                ),
                "runtime_action": _runtime_action_for_cpu_fallback(policy),
                **_probe_result_payload(after),
            }

    return {
        "selected_backend": "cpu",
        "fallback_reason": before.error or last_error or f"unable to install a {policy.backend or 'GPU'}-capable runtime",
        "runtime_action": _runtime_action_for_install_failure(policy),
        **_probe_result_payload(before),
    }
