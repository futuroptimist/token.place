...
def test_public_information_exemption_does_not_cover_mutation_methods(path, method):
    """Exact public-information paths remain quota-protected for mutations."""
    app = Flask(__name__)
    init_app(app)
    app.add_url_rule(
        path,
...
def test_public_information_lookalikes_remain_rate_limited(path):
    """Only normalized exact allowlist matches receive the safe-read exemption."""
    app = Flask(__name__)
    init_app(app)
    app.add_url_rule(
        path,
        endpoint=f"public-information-lookalike-{path}",
        view_func=lambda: {"status": "ok"},
        methods=["GET"],
    )
...
def test_public_information_reads_use_existing_path_normalization(path):
    """Trailing slashes remain equivalent under the existing normalizer."""
    app = Flask(__name__)
    init_app(app)
    app.add_url_rule(
        path,
        endpoint=f"normalized-public-information-{path}",
        view_func=lambda: {"status": "ok"},
        methods=["GET"],
    )
