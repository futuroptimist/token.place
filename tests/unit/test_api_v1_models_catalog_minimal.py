"""Regression tests ensuring API v1 exposes only the canonical Qwen3 8B model."""
...
def test_api_v1_catalog_is_restricted_to_single_qwen3_8b_model():
    import api.v1.models as models

    importlib.reload(models)

    entries = models.get_models_info()
    assert [entry["id"] for entry in entries] == ["qwen3-8b-instruct"]

    base_entry = entries[0]
    assert base_entry["base_model_id"] == "qwen3-8b-instruct"
    assert base_entry["owned_by"] == "Qwen"
    assert base_entry["owner"] == "Qwen"
    assert base_entry["provider"] == "qwen"
    assert base_entry["source_model"] == "Qwen/Qwen3-8B"
    assert "adapter" not in base_entry


@mock.patch.dict(os.environ, {"USE_MOCK_LLM": "1"})
def test_qwen3_catalog_targets_default_q4_k_m_release():
    import api.v1.models as models

    importlib.reload(models)

    base_entry = next(
        entry for entry in models.get_models_info() if entry["id"] == "qwen3-8b-instruct"
    )

    assert base_entry["name"] == "Qwen3 8B Instruct"
    assert base_entry["file_name"] == "Qwen3-8B-Q4_K_M.gguf"
    assert base_entry["file_name"] in base_entry["url"]
    assert base_entry["quantization"] == "Q4_K_M"
    assert base_entry["license"] == "apache-2.0"


def test_qwen3_profile_is_public_catalog_default_with_64k_yarn():
    ...
    assert qwen["public_catalog"] is True
