...
def desktop_runner():
    ...
    assignments = {
        ...
        "RELAY_DIAGNOSTICS_USER_AGENT", "RELAY_PROBE_FAILURE_CATEGORIES",
    ...
    names = {"_wait_for_packaged_setup_condition", "_prepare_packaged_landing_page",
        ...
        "fetch_api_v1_registered_node_fingerprints", "fetch_relay_diagnostics_count",
        "_fetch_relay_diagnostics_payload", "_relay_probe_error",
        "_bridge_reset_observations",
    ...
    namespace.update({"webdriver": SimpleNamespace(Chrome=object, Remote=object,
        ...
        "urlopen": None, "Request": __import__("urllib.request", fromlist=["Request"]).Request,
...
def test_webdriver_diagnostic_clamps_hostile_pre_start_fields(desktop_runner, tmp_path):
    ...
    desktop_runner._write_webdriver_diagnostic(
        "match", "running", "none", pre_start_diagnostic={
            ...
            "baseline_failure_category_counts": {
                "http_status": 10**20, "timeout": True,
                "C:\\private\\SECRET": 12},
    ...
    expected["baseline_failure_category_counts"] = {
        **expected["baseline_failure_category_counts"],
...
def test_relay_diagnostics_transport_uses_explicit_honest_user_agent(
    ...
    def transport(request, *, timeout):
        ...
        # Deterministically model the observed default-header rejection.
        if request.get_header("User-agent") != desktop_runner.RELAY_DIAGNOSTICS_USER_AGENT:
            raise desktop_runner.HTTPError(request.full_url, 403, "rejected", {}, None)
        return Response()

    desktop_runner.urlopen = transport
...
@pytest.mark.parametrize("failure, category, status", [
    (lambda runner: runner.HTTPError("https://relay.example", 403, "private", {}, None),
        "http_status", 403),
    (lambda runner: runner.URLError(runner.socket.gaierror("private")), "dns", None),
    (lambda runner: runner.URLError(ConnectionRefusedError("private")), "connection", None),
    (lambda runner: runner.URLError(runner.ssl.SSLError("private")), "tls", None),
    (lambda runner: TimeoutError("private"), "timeout", None),
])
def test_relay_diagnostics_transport_failures_are_bounded(
        desktop_runner, failure, category, status):
    desktop_runner.urlopen = lambda *_args, **_kwargs: (_ for _ in ()).throw(
        failure(desktop_runner))
    with pytest.raises(ValueError, match="^relay diagnostics probe failed$") as raised:
        desktop_runner.fetch_relay_diagnostics_count(
            "https://relay.example/private", timeout_seconds=0.5)
    assert raised.value.relay_probe_category == category
    assert raised.value.relay_http_status == status
    assert "private" not in str(raised.value)
...
def test_relay_baseline_records_repeated_http_status_without_private_data(
        desktop_runner, monkeypatch):
    clock = [0.0]
    def monotonic():
        clock[0] += 0.03
    ...
    monkeypatch.setattr(desktop_runner.time, "monotonic", monotonic)
    monkeypatch.setattr(desktop_runner.time, "sleep", lambda _seconds: None)
    private = "C:\\Users\\private\\prompt-secret"
    desktop_runner.fetch_api_v1_registered_node_fingerprints = \
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            desktop_runner._relay_probe_error("http_status", 403, private))
    diagnostic = desktop_runner.PRE_START_DIAGNOSTIC_DEFAULTS.copy()
    with pytest.raises(RuntimeError, match="^operator_registration_not_reached$"):
        desktop_runner.require_clean_relay_registration_baseline(
            "https://relay.example", timeout_seconds=0.1,
