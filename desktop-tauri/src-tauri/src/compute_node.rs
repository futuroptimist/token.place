...
mod tests {
    ...
    #[test]
    fn operator_log_path_preserves_safe_qwen_64k_token_metadata() {
        let payload = serde_json::json!({
            "type": "status",
        ...
        let stdout_event: Value = serde_json::from_str(&stdout).expect("stdout event json");
        ...
        for chunk in chunks {
            ...
            let event: Value = serde_json::from_str(&sanitized).expect("readiness event json");
            let event_diagnostics = event
                .get("diagnostics")
                .and_then(Value::as_object)
                .expect("diagnostics object");
            for (key, value) in event_diagnostics {
                diagnostics.insert(key.clone(), value.clone());
            }
            assert!(!sanitized.contains("SECRET_ACCESS_TOKEN"));
        }
