...
def _trusted_configured_relay_origins() -> list[tuple[str, str]]:
    """Return relay origins trusted for request-scoped same-origin routing."""

    config = get_config()
    candidates: list[tuple[str, str]] = [
        ("config:relay.server_url", config.get("relay.server_url", "")),
        ("config:api.relay_url", config.get("api.relay_url", "")),
    ]
    for item in config.get("relay.server_pool", []) or []:
        candidates.append(("config:relay.server_pool", item))
...
def _request_relay_target_selection() -> DistributedTargetSelection:
    ...
    env_selection = select_api_v1_distributed_target()
    ...
    for trusted_origin, source in _trusted_configured_relay_origins():
        if trusted_origin == "https://token.place" and token_place_env != "production":
            continue
        return DistributedTargetSelection(
            url=trusted_origin,
            source=source if trusted_origin != "https://token.place" else "production_default",
            relay_only=False,
        )
...
def _handle_chat_completion_request(data):
    ...
    try:
        ...
        if force_desktop_bridge_distributed:
            relay_target_selection = _request_relay_target_selection()
            provider = get_api_v1_compute_provider_for_mode(
                mode="distributed",
                distributed_url=relay_target_selection.url,
                distributed_fallback_enabled=False,
                distributed_target_source=relay_target_selection.source,
                distributed_relay_only=relay_target_selection.relay_only,
            )
        else:
        ...
        if is_encrypted_request and client_public_key:
            ...
            response.headers["X-Tokenplace-API-V1-Execution-Backend-Path"] = execution_backend_path
