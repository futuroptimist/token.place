...
The release workflow cancels older in-progress runs for the same effective tag/ref and keeps
temporary workflow artifacts for seven days. Published GitHub Release assets are unaffected by that
temporary artifact retention window.
...
single model, `qwen3-8b-instruct` (Qwen3 8B Q4_K_M), and the desktop runtime reuses one warmed llama.cpp runtime
