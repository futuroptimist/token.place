interface RelayProgress { schema_version?: number; sequence?: number; phase: string; total_prompt_tokens: number; cached_prompt_tokens: number; processed_prompt_tokens: number; generated_tokens: number; elapsed_ms: number; }
interface ProgressEnvelope { protocol: string; version: number; request_id: string; client_public_key: string; api_v1_progress: RelayProgress; }

const counters: (keyof RelayProgress)[] = ["total_prompt_tokens", "cached_prompt_tokens", "processed_prompt_tokens", "generated_tokens", "elapsed_ms"];

function relayProgressText(progress: RelayProgress | null): string {
...
function validProgressEnvelope(envelope: ProgressEnvelope, requestId: string, clientKey: string, lastSequence: number): boolean {
    ...
    return Boolean(progress && envelope.protocol === "tokenplace_api_v1_relay_e2ee" && envelope.version === 1
        ...
        && progress.schema_version === 1 && Number.isSafeInteger(progress.sequence) && (progress.sequence as number) > lastSequence
        && ["preparing", "prefill", "generating"].includes(progress.phase)
        && counters.every((key) => Number.isSafeInteger(progress[key]) && (progress[key] as number) >= 0)
...
(window as unknown as Window & { TokenPlaceProgress: unknown }).TokenPlaceProgress = { relayProgressText, validProgressEnvelope };
