...
def launch_webview2_application(app_binary: Path, env: dict[str, str],
        output, timeout_seconds: float, *, application_args: list[str] | None = None,
        process_started: Callable[[subprocess.Popen[str]], None] | None = None
        ) -> tuple[subprocess.Popen[str], str]:
    """Launch an owned packaged app and return its attachable DevTools address."""
    ...
    resolved_binary = app_binary.resolve(strict=True)
    try:
        application_process = subprocess.Popen(
            [str(resolved_binary), webview2_automation_arg,
                *list(application_args or [])],
            cwd=resolved_binary.parent, env=application_env,
            stdout=output, stderr=subprocess.STDOUT, text=True)  # noqa: S603
    except (OSError, ValueError):
        raise RuntimeError("webdriver_application_startup_failed") from None
    try:
        if process_started is not None:
            process_started(application_process)
...
def run_long_context_packaged_mode(request_path: Path, evidence_path: Path,
    ...
    try:
        ...
        if os.name == "nt":
            def start_memory_sampling(launched_process) -> None:
                nonlocal memory_sampler
                memory_sampler = OwnedProcessTreeMemorySampler(launched_process.pid)
            try:
                application_process, debugger_address = launch_webview2_application(
                    app_binary, env, driver_log_handle, setup_remaining(),
                    application_args=application_args,
                    process_started=start_memory_sampling)
...
def main(argv: list[str] | None = None) -> int:
    ...
    env["WEBVIEW2_USER_DATA_FOLDER"] = str(isolated_home / "WebView2")
    Path(env["WEBVIEW2_USER_DATA_FOLDER"]).mkdir(parents=True, exist_ok=True)
    ...
    try:
        ...
        if hardware_mode:
            application_process, debugger_address = launch_webview2_application(
                app_binary, env, driver_log.open("a", encoding="utf-8"), 45)
