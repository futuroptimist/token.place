...
def require_clean_relay_registration_baseline(
        ...
        record_relay_observation, record_pre_start_diagnostic=lambda _field, _value: None
        ) -> list[str]:
    """Require an authoritative empty relay before starting this attempt."""
    deadline = time.monotonic() + timeout_seconds
    attempts = 0
    transient_failures = 0
    record_pre_start_diagnostic("baseline_outcome", "polling")
    while True:
        remaining = deadline - time.monotonic()
        if remaining <= 0:
            record_pre_start_diagnostic(
                "baseline_outcome",
                "probe_failure_exhausted" if transient_failures else "deadline_exhausted")
            fail_closed("operator_registration_not_reached")
        attempts += 1
        record_pre_start_diagnostic("baseline_poll_attempt_count", attempts)
        ...
        try:
            ...
            record_pre_start_diagnostic("baseline_transient_failure_count", transient_failures)
            time.sleep(min(0.1, max(0.0, deadline - time.monotonic())))
            continue
        record_pre_start_diagnostic(
            "baseline_last_authoritative_registered_node_count", len(registered))
        if registered:
            record_pre_start_diagnostic("baseline_outcome", "authoritative_nonzero_rejected")
            record_relay_observation("not_reached")
            fail_closed("operator_registration_not_reached")
        record_pre_start_diagnostic("baseline_outcome", "authoritative_zero_accepted")
        return registered
...
PRE_START_DIAGNOSTIC_ALLOWLISTS = {
    "baseline_outcome": frozenset({
        ...
        "authoritative_nonzero_rejected", "probe_failure_exhausted",
        "deadline_exhausted",
    }),
    "start_click_state": frozenset({
        "not_reached", "about_to_attempt", "returned", "exception",
        "native_handler_entered",
    }),
    "start_click_exception_category": frozenset({
        "none", "element_not_found", "stale_element", "webdriver_failure", "unknown",
    }),
}
...
def _finalize_pre_start_diagnostic(
        supplied: dict[str, object] | None,
        operator_start: dict[str, str]) -> dict[str, object]:
    """Clamp credential-safe harness state and incorporate native handler entry."""
    supplied = supplied or {}
    result: dict[str, object] = {
        field: (value if (value := supplied.get(field)) in allowed
                else PRE_START_DIAGNOSTIC_DEFAULTS[field])
        for field, allowed in PRE_START_DIAGNOSTIC_ALLOWLISTS.items()
    }
    for field in (
            "baseline_poll_attempt_count", "baseline_transient_failure_count"):
        value = supplied.get(field)
        result[field] = value if isinstance(value, int) and not isinstance(value, bool) \
            and value >= 0 else PRE_START_DIAGNOSTIC_DEFAULTS[field]
    count = supplied.get("baseline_last_authoritative_registered_node_count")
    result["baseline_last_authoritative_registered_node_count"] = (
        count if isinstance(count, int) and not isinstance(count, bool) and count >= 0
        else None)
    if operator_start.get("start_handler_state") == "entered":
        result["start_click_state"] = "native_handler_entered"
    return result


def _click_start_operator(driver: object, record_pre_start_diagnostic) -> None:
    """Record the exact WebDriver click boundary without retaining exception text."""
    record_pre_start_diagnostic("start_click_state", "about_to_attempt")
    try:
        driver.find_element(By.XPATH, "//button[.='Start operator']").click()
    except Exception as exc:
        record_pre_start_diagnostic("start_click_state", "exception")
        category = (
            "element_not_found" if isinstance(exc, NoSuchElementException) else
        ...
        record_pre_start_diagnostic("start_click_exception_category", category)
        raise
    record_pre_start_diagnostic("start_click_state", "returned")
...
def _write_webdriver_diagnostic(
    ...
    safe_pre_start_diagnostic = _finalize_pre_start_diagnostic(
        pre_start_diagnostic, safe_start_diagnostic)
    LOGS_DIR.mkdir(parents=True, exist_ok=True)
...
def run_long_context_packaged_mode(request_path: Path, evidence_path: Path,
    ...
    def record_pre_start_diagnostic(field: str, value: object) -> None:
        if field in PRE_START_DIAGNOSTIC_DEFAULTS:
            pre_start_diagnostic[field] = value
    try:
        setup_remaining()
        try:
        ...
        _click_start_operator(driver, record_pre_start_diagnostic)
