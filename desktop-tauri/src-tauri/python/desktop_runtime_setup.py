...
def _qwen_64k_runtime_repair_failed_reason(probe: RuntimeProbe) -> str:
    prefix = (
        "runtime_qwen_64k_yarn_rope_freq_scale_unavailable; "
        if not probe.rope_freq_scale_supported
        else ""
    )
    return (
        f"{prefix}Qwen 64K requires YaRN/RoPE support in llama-cpp-python; runtime repair failed; "
