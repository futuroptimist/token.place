...
## Desktop versioning policy
- Major and minor desktop version changes require explicit maintainer instructions. A functional
  change to shipped desktop behavior, packaging, installation/upgrades, native code, UI, embedded
  sidecars, bundled dependencies, or shared runtime code included in desktop must default to a
  patch bump in the same PR.
- When applicability is ambiguous, bump the patch version if users need a newly built installer to
  receive or validate the change. A coordinated multi-PR release train may defer this only when the
  task or PR record explicitly assigns the bump elsewhere; never silently assume another PR will.
- Relay-only, web-only, documentation-only, test-only, CI-only, and behavior-preserving refactors do
  not require a bump unless they alter the desktop artifact or require a rebuilt installer.
- A patch bump must update every synchronized version surface and version-dependent fixture. It does
  not authorize tagging, publishing, or creating a release. The commit/build ID identifies an exact
  build; the semantic version identifies an intentionally packaged desktop release.
