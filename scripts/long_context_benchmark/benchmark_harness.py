...
def validate_report(report: Any) -> None:
    ...
    if report["status"] != "passed" and "runtime" not in report:
        ...
        if report["code"] == "packaged_runner_timeout":
            ...
            allowed = {"last_safe_phase", "failure_reason", "elapsed_s", "cleanup_succeeded"}
            present = allowed.intersection(report)
            if (not {"last_safe_phase", "failure_reason", "elapsed_s"}.issubset(present)
                    or report["last_safe_phase"] not in PACKAGED_PHASES
                    or report["failure_reason"] not in PACKAGED_FAILURE_REASONS
                    or not finite(report["elapsed_s"]) or report["elapsed_s"] < 0
                    or ("cleanup_succeeded" in report
                        and not isinstance(report["cleanup_succeeded"], bool))):
...
def _read_packaged_phase_status(path: Path, parent_elapsed_s: float) -> tuple[dict[str, Any] | None, str | None]:
    ...
    if (value["last_safe_phase"] not in PACKAGED_PHASES
            or value["failure_reason"] not in PACKAGED_FAILURE_REASONS | {None}
            or value["cleanup_succeeded"] not in {True, False, None}):
        return None, "packaged_phase_status_malformed"
    return value, None
...
def invoke_packaged_runtime_adapter(*, fixture_id: str = "small-8k", scenario: str = "structured-extraction", timeout_s: float = 30.0,
    ...
    try:
        ...
        if completed.returncode != 0:
            ...
            if phase_error:
                return {"pass": False, "runtime_contract_pass": False, "code": phase_error}
            result = {"pass": False, "runtime_contract_pass": False,
                "code": "packaged_runner_failed", "last_safe_phase": status["last_safe_phase"],
                "failure_reason": status["failure_reason"] or "runner_failure",
                "elapsed_s": elapsed_s}
            if status["cleanup_succeeded"] is not None:
                result["cleanup_succeeded"] = status["cleanup_succeeded"]
            return result
        try:
