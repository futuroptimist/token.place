...
class EncryptedProgressEnvelope:
    """Exact API-v1 relay-blind progress ciphertext allowlist."""
...
class ProgressRecord:
    """Latest advisory progress for one live canonical request identity."""
    ...
    def __repr__(self) -> str:
        return (
            "ProgressRecord(identity=<redacted>, envelope=<redacted>, "
            f"expires_at_epoch={self.expires_at_epoch!r})"
        )
...
class ProgressReplacementResult:
    """Payload- and identity-free progress replacement result."""
...
class InMemoryRelayStateStore:
    ...
    def retrieve_encrypted_response(
        ...
        with self._lock:
            ...
            if terminal is None:
                queued = self._queued.get(identity)
                expected_digest = self._queued_token_digests.get(identity)
                if (
                    queued is not None
    ...
    def _reap_locked(self, now: float) -> None:
        ...
        self._reap_retained_locked(now)
        for identity, progress in tuple(self._progress.items()):
            claim = self._claims.get(identity)
            if (
                progress.expires_at_epoch <= now
                or claim is None
                or claim.lease_expires_at_epoch <= now
            ):
                del self._progress[identity]
