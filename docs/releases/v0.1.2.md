...
## Prompt 06 implementation note: relay metrics slice

This repository now includes an application-level, relay-blind Prometheus metrics
slice for the single-pod, one-worker, in-memory relay runtime. This section is an
implementation note only; it does not mark Sugarkube staging or production scrape
requirements complete without live cluster evidence.

### Exported metrics

- `tokenplace_http_requests_total`: counter labelled by `method`, normalized
  `route`, `status_class`, `provider_mode`, and `outcome`.
- `tokenplace_http_request_duration_seconds`: histogram with the same bounded
  HTTP labels and coarse latency buckets from 5 ms through 60 s.
- `tokenplace_relay_queue_depth`: gauge labelled only by bounded
  `provider_mode`; currently `api_v1_relay` is populated.
- `tokenplace_relay_oldest_queued_request_age_seconds`: aggregate gauge that is
  `0` when no encrypted work is queued.
- `tokenplace_compute_nodes_registered`: aggregate gauge of API v1 compute-node
  registrations held in process memory.
- `tokenplace_compute_nodes_healthy`: aggregate gauge using the relay's existing
  lease and long-poll staleness semantics.
- `tokenplace_compute_node_lease_age_seconds`: aggregate gauge for the oldest
  registered API v1 compute-node lease age; no node identity labels are used.
- `tokenplace_compute_node_evictions_total`: counter labelled by fixed eviction
  `reason`.
- `tokenplace_relay_in_flight_requests`: aggregate gauge of API v1 encrypted
  requests dispatched to compute nodes but not yet terminal.
- `tokenplace_relay_oldest_in_flight_age_seconds`: aggregate gauge that is `0`
  when no API v1 request is in flight.
- `tokenplace_relay_request_outcomes_total`: counter labelled by fixed terminal
  `outcome`.
- `tokenplace_build_info`: build identity gauge fixed at `1`, labelled by
  `version` and `revision` from release metadata/environment helpers.
- `tokenplace_instrumentation_up`: gauge set to `1` after metrics initialize.

The legacy compatibility counter `tokenplace_relay_requests_total{method,endpoint,status}`
remains exported so existing dashboards can continue to scrape it while moving to
the bounded HTTP counter and histogram.

### Fixed label enums

- `provider_mode`: `api_v1_relay`, `relay`, `local`, `unknown`.
- HTTP `outcome`: `success`, `client_error`, `rate_limited`, `server_error`.
- Relay terminal `outcome`: `completed`, `cancelled`, `expired`, `timed_out`,
  `rate_limited`, `dependency_failure`, `failed`.
- Compute-node eviction `reason`: `stale_lease`, `explicit_unregister`.
- `status_class`: standard status families such as `2xx`, `4xx`, and `5xx`.

### Privacy and cardinality invariants

Metrics must remain relay-blind. Metric names and labels never include plaintext
prompts, plaintext responses, ciphertext, keys, public keys, request IDs, client
IDs, node IDs, IP addresses, auth headers, model names, arbitrary URLs, user
agents, raw errors, or payload-derived sizes. Identities are not hashed into
labels; queue, in-flight, lease, and capacity metrics aggregate instead.

`/metrics` remains exempt from the public API rate limits, but when
`TOKENPLACE_METRICS_TOKEN` is set it requires `Authorization: Bearer <token>` and
uses constant-time comparison for the token check.

### PromQL examples

```promql
sum by (status_class, outcome) (rate(tokenplace_http_requests_total[5m]))
```

```promql
tokenplace_relay_queue_depth{provider_mode="api_v1_relay"}
```
...
```promql
sum by (outcome) (increase(tokenplace_relay_request_outcomes_total[15m]))
```
