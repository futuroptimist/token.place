...
API_V1_MAX_QUEUE_DEPTH_PER_SERVER_ENV = "TOKEN_PLACE_API_V1_MAX_QUEUE_DEPTH_PER_SERVER"
DEFAULT_API_V1_MAX_QUEUE_DEPTH_PER_SERVER = 128
...
def _api_v1_max_queue_depth_per_server() -> int:
    raw = os.environ.get(API_V1_MAX_QUEUE_DEPTH_PER_SERVER_ENV, str(DEFAULT_API_V1_MAX_QUEUE_DEPTH_PER_SERVER))
    try:
        value = int(raw)
    except ValueError:
        return DEFAULT_API_V1_MAX_QUEUE_DEPTH_PER_SERVER
    return max(value, 1)
...
def _live_server_diagnostics(*, api_v1_only: bool = False) -> list[dict[str, Any]]:
    ...
    for server_public_key, payload in list(known_servers.items()):
        if api_v1_only and not bool(payload.get(API_V1_SERVER_MARKER)):
            continue
        load = _api_v1_server_load(server_public_key, payload) if bool(payload.get(API_V1_SERVER_MARKER)) else {
...
def _api_v1_prune_in_flight_for_payload(payload: dict[str, Any], *, now_monotonic: float | None = None) -> int:
    """Prune expired API v1 in-flight markers and return the active count."""
...
def _api_v1_server_load(server_public_key: str, payload: dict[str, Any]) -> dict[str, Any]:
    """Return safe load metadata for relay-blind API v1 scheduling."""
    ...
    if not isinstance(max_concurrency, int) or max_concurrency < 1:
        max_concurrency = 1
    queue_depth = len(client_inference_requests.get(server_public_key, []))
    in_flight_count = _api_v1_prune_in_flight_for_payload(payload)
    ...
    return {
        ...
        "max_concurrency": max_concurrency,
        "max_queue_depth": _api_v1_max_queue_depth_per_server(),
    }


def _api_v1_server_is_saturated(load: dict[str, Any]) -> bool:
    return (
        load["in_flight_count"] >= load["max_concurrency"]
        or load["queue_depth"] >= load["max_queue_depth"]
    )
...
def _api_v1_scheduler_candidate(
    ...
    *,
    ...
    """Return a safe scheduling candidate or None when the node is ineligible."""
    ...
    if active_tier not in CONTEXT_TIER_ORDER:
        return None
    ...
    if requested_model and requested_model not in supported_models:
        return None
    try:
        stale_after = float(payload.get("last_ping_duration", _server_stale_seconds()) or 0)
    except (TypeError, ValueError):
        stale_after = float(_server_stale_seconds())
    if _server_ping_age_seconds(payload.get("last_ping")) > max(stale_after, 1.0):
        return None
    load = _api_v1_server_load(server_public_key, payload)
    if _api_v1_server_is_saturated(load):
        return None
...
def _select_best_fit_api_v1_server_key(*, model: str | None = None, context_tier: str = DEFAULT_CONTEXT_TIER) -> tuple[str | None, dict[str, Any], dict[str, Any] | None]:
    """Select the smallest capable API v1 node, then least-loaded within that tier."""
    ...
    with server_round_robin_lock:
        ...
        candidates = []
        for index, server_public_key in enumerate(all_api_v1_keys):
        ...
        tied = [candidate for candidate in tier_candidates if candidate["load_score"] == min_load]
        tied.sort(key=lambda candidate: candidate["registration_index"])

        tie_key = tuple(candidate["server_public_key"] for candidate in tied)
        if tie_key in api_v1_filtered_round_robin_next_positions:
            tie_index = api_v1_filtered_round_robin_next_positions[tie_key] % len(tied)
        else:
            current_index = server_round_robin_next_index % registered_count if registered_count else 0
            tie_index = next(
                (
                    tied.index(candidate)
                    for offset in range(registered_count)
                    for candidate in tied
                    if candidate["registration_index"] == (current_index + offset) % registered_count
                ),
                0,
            )
        selected = tied[tie_index]
        api_v1_filtered_round_robin_next_positions[tie_key] = (tie_index + 1) % len(tied)
        if registered_count:
        ...
        selection = {
            ...
            "selection_policy": API_V1_SELECTION_POLICY,
            "spillover": spillover,
            "spillover_reason": "no_healthy_unsaturated_smaller_tier" if spillover else None,
        }
...
def _select_next_server_payload(*, api_v1: bool = False):
    ...
    LOGGER.info(
        ...
        extra={
            ...
            "requested_context_tier": context_tier,
            "requested_model": model,
            "eligible_node_count": selection.get("eligible_node_count"),
    ...
    if selection.get("spillover_reason"):
        response_payload['spillover_reason'] = selection.get("spillover_reason")
