...
- Scan staged changes for credentials with
  `git diff --cached | ./scripts/scan-secrets.py` (or the repository-approved alternative)
  prior to committing.
...
- Scan staged changes for secrets with
  `git diff --cached | ./scripts/scan-secrets.py` (or the approved substitute) before
  committing.
...
- Perform the standard secret scan via
  `git diff --cached | ./scripts/scan-secrets.py` (or the approved equivalent).
