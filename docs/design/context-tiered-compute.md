...
Qwen 64K operators can select **Balanced** (the recommended default), **Safe** (the conservative P4-equivalent setting), or **Experimental** (explicit opt-in only). Changing the saved selection requires stopping and restarting the operator; it is retained but not applied for the 8K tier. On recognized memory pressure, the runtime downshifts Experimental to Balanced to Safe at the current KV precision before using Safe Q4. Q8 compatibility failures instead select same-batch F16; subsequent memory pressure then downshifts the F16 batch class before Safe Q4. Other failures do not reduce batches or select Q4. Performance varies by backend and hardware, and P8 will supply formal comparative benchmarks and regression thresholds.
