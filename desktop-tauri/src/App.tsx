...
function normalizeQwen64kBatchProfile(value: unknown): Qwen64kBatchProfile {
  return value === 'safe' || value === 'balanced' || value === 'experimental' ? value : 'balanced';
}
  ...
  return (
    <main style={{ maxWidth: 820, margin: '20px auto', fontFamily: 'sans-serif' }}>
      ...
      <section style={{ marginTop: 14, border: '1px solid #ddd', padding: 12 }}>
        ...
        <label htmlFor="qwen-64k-batch-profile" style={{ display: 'block', marginBottom: 8 }}>
          Qwen 64K batch profile
        </label>
        <select
          id="qwen-64k-batch-profile"
        ...
        <p id="qwen-64k-batch-profile-help" style={{ marginTop: 4, fontSize: 12, color: '#555' }}>
          {config.context_tier === '64k-full' ? 'Balanced: 512/256; Safe: 256/128; Experimental: 1024/512. Stop and restart the operator to apply changes. Experimental can increase memory use or instability.' : 'Inactive for the 8K tier; the saved 64K preference is not applied.'}
        </p>
        <div style={{ display: 'flex', gap: 8 }}>
