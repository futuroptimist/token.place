...
def _classify_generation_exception(exc):
    ...
    if any(term in text for term in ('decode failed', 'llama_decode', 'decode returned')):
        return 'prompt_eval_decode_failure'
    if any(term in text for term in ('backend', 'metal command buffer', 'cuda error')):
    ...
    if any(term in text for term in ('state', 'reset', 'sequence', 'batch')):
        return 'prompt_eval_state_failure'
    if any(term in text for term in ('context', 'n_ctx', 'ctx')) and any(term in text for term in ('eval', 'evaluate', 'decode')):
...
def _tokenize_rendered_prompt_variants_for_plain_completion(llama, rendered_prompt):
    ...
    attempts = []
    if supports_special:
        attempts.append(('tokenize_add_bos_false_special_false', False, False, {'add_bos': False, 'special': False}))
    attempts.append(('tokenize_add_bos_false_no_special', None, False, {'add_bos': False}))
    if supports_special:
        attempts.append(('tokenize_add_bos_false_special_true', True, False, {'add_bos': False, 'special': True}))
    variants = []
    seen = set()
    last_category = 'prompt_tokenization_failure'
    for variant_id, special_value, add_bos, kwargs in attempts:
        try:
    ...
    diagnostics['plain_completion_prompt_tokenization_variant_count'] = len(variants)
    diagnostics['plain_completion_prompt_tokenization_variant_ids'] = ','.join(v['tokenization_variant_id'] for v in variants)
    diagnostics['plain_completion_prompt_tokenization_token_counts'] = ','.join(str(v['token_count']) for v in variants)
    diagnostics['plain_completion_prompt_tokenization_special_values'] = ','.join('none' if v['special'] is None else str(bool(v['special'])).lower() for v in variants)
    if variants:
        selected = variants[0]
        diagnostics['plain_completion_prompt_token_count'] = selected['token_count']
        diagnostics['plain_completion_prompt_tokenization_method'] = selected['method']
        diagnostics['plain_completion_prompt_tokenization_special'] = selected['special']
        diagnostics['plain_completion_prompt_tokenization_selected_variant'] = selected['tokenization_variant_id']
...
def _tokenize_rendered_prompt_for_plain_completion(llama, rendered_prompt):
    ...
    return (variants[0]['token_ids'] if variants else None), diagnostics
        ...
        if method == 'create_chat_completion_from_rendered_prompt':
            ...
            if result is not None:
                ...
                if invalid_reason == 'thinking_leaked':
                    attempts[-1]['generation_exception_category'] = invalid_reason
            if (result is None or (invalid_reason is not None and invalid_reason != 'thinking_leaked')) and callable(create_completion) and not _last_fatal():
                ...
                for variant in tokenization_variants:
                    if result is not None and invalid_reason is None:
                        break
                    ...
                    attempts[-1]['tokenization_variant'] = variant_id
                    if result is not None:
                        normalized, invalid_reason = _normalize_plain_completion_result(result)
                        if invalid_reason == 'thinking_leaked':
                            attempts[-1]['generation_exception_category'] = invalid_reason
            ...
            if result is not None and invalid_reason is None:
                pass
            elif (result is None or invalid_reason not in {'thinking_leaked'}) and not _last_fatal():
                chat_completion = getattr(llama, 'create_chat_completion', None)
                supported = False
                rejected = ''
                try:
                    sig = inspect.signature(chat_completion) if callable(chat_completion) else None
                    if sig is not None:
                        supported = 'chat_template_kwargs' in sig.parameters or any(p.kind == inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values())
                except Exception:
                    supported = False
                plain_capabilities.update({
                    'qwen_high_level_chat_fallback_attempted': bool(callable(chat_completion) and supported),
                    'qwen_high_level_chat_fallback_supported': bool(supported),
                    'qwen_high_level_chat_fallback_succeeded': False,
                    'qwen_high_level_chat_fallback_rejected_kwarg': '',
                    'qwen_high_level_chat_fallback_category': '',
                })
                if callable(chat_completion) and supported:
                    try:
                        chat_result = chat_completion(*request.get('args', []), max_tokens=max_tokens, chat_template_kwargs={'enable_thinking': False})
                        attempts.append({'method': 'qwen_high_level_chat_fallback', 'attempted_kwarg_names': 'chat_template_kwargs,max_tokens', 'result_shape': _completion_result_shape(chat_result)})
                        normalized, invalid_reason = _normalize_plain_completion_result(chat_result)
                        result = chat_result
                        ...
                        category = _plain_completion_method_shape_category(exc)
                        rejected = _extract_unsupported_generation_kwarg(str(exc), ['chat_template_kwargs', 'max_tokens']) or ''
                        plain_capabilities['qwen_high_level_chat_fallback_rejected_kwarg'] = rejected
                        plain_capabilities['qwen_high_level_chat_fallback_category'] = category
                        attempts.append({'method': 'qwen_high_level_chat_fallback', 'attempted_kwarg_names': 'chat_template_kwargs,max_tokens', 'exception_type': type(exc).__name__, 'generation_exception_category': category, 'rejected_generation_kwarg': rejected, 'sanitized_error_summary': _sanitize_error_summary(exc)})
            plain_capabilities.update({
                'plain_completion_attempt_methods': ','.join(item.get('method', '') for item in attempts if item.get('method')),
                'plain_completion_attempt_categories': ','.join(item.get('generation_exception_category', '') for item in attempts if item.get('generation_exception_category')),
                'plain_completion_attempt_exception_types': ','.join(item.get('exception_type', '') for item in attempts if item.get('exception_type')),
                'plain_completion_attempt_safe_summaries': ','.join(item.get('sanitized_error_summary', '') for item in attempts if item.get('sanitized_error_summary')),
                'plain_completion_attempt_rejected_kwargs': ','.join(item.get('rejected_generation_kwarg', '') for item in attempts if item.get('rejected_generation_kwarg')),
                'plain_completion_attempt_result_shapes': ','.join(item.get('result_shape', '') for item in attempts if item.get('result_shape')),
                'plain_completion_attempt_tokenization_variants': ','.join(item.get('tokenization_variant', '') for item in attempts if item.get('tokenization_variant')),
