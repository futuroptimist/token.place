...
def test_landing_chat_js_avoids_relay_v2_streaming_path():
    ...
    assert "/api/v1/relay/servers/next" in chat_js, (
        "landing chat must select an API v1 relay compute node directly"
    )
    assert "/api/v1/relay/requests" in chat_js, (
        "landing chat must queue direct API v1 relay E2EE envelopes"
    )
    assert "/api/v1/relay/responses/retrieve" in chat_js, (
        "landing chat must poll direct API v1 relay E2EE responses"
    )
    assert "/api/v1/chat/completions" not in chat_js, (
        "landing chat must not call server-selected API v1 chat completions endpoint"
    )
