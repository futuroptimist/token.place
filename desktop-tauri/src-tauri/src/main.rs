...
async fn start_inference(
    ...
    tokio::spawn(async move {
        if let Err(err) = sidecar::start_sidecar(app.clone(), sidecar_state, request).await {
            eprintln!(
                "desktop.sidecar.start_error request_id={} {}",
                request_id, err
            );
            let _ = app.emit(
                "inference_event",
...
async fn start_compute_node(
    ...
    tokio::spawn(async move {
        if let Err(err) =
            compute_node::start_compute_node(app.clone(), compute_state, request).await
        {
            eprintln!("desktop.compute_node.start_error {}", err);
            let _ = app.emit(
                "compute_node_event",
                serde_json::json!({
                    "type": "error",
