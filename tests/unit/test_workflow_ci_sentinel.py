...
def test_desktop_release_workflow_is_tag_or_manual_only_with_release_concurrency() -> None:
    ...
    assert "pull_request" not in on_block, (
        "Desktop release packaging is too expensive for ordinary PRs; it must run only "
        "for immutable desktop-v* tags or intentional manual dispatches."
    )
    assert on_block["push"]["tags"] == ["desktop-v*"]
    assert "workflow_dispatch" in on_block
    assert workflow_data["concurrency"] == {
        "group": "desktop-release-${{ inputs.tag_name || github.ref_name }}",
...
def test_desktop_release_temporary_artifacts_expire_after_seven_days() -> None:
    ...
    upload_steps = [
        step
        for step in _job_steps(workflow_data["jobs"]["build"])
        if step.get("uses") == "actions/upload-artifact@v5"
    ]

    assert upload_steps, "desktop-release.yml must upload temporary build artifacts"
    assert all(
        step.get("with", {}).get("retention-days") == "7" for step in upload_steps
    ), "Temporary release workflow artifacts must expire after seven days"
...
def test_run_all_tests_pr_workflow_has_visible_linux_full_suite_only() -> None:
    ...
    assert not any(
        job.get("runs-on") != "ubuntu-latest" for job in jobs.values()
    ), "run_all_tests PR workflow must not add replacement non-Linux full-suite jobs"


def test_run_all_tests_pr_linux_job_invokes_the_full_suite() -> None:
