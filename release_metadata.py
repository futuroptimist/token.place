...
def resolve_asset_version(metadata: dict[str, str] | None = None) -> str:
    """Resolve a short public cache-busting token for frontend assets."""

    metadata = metadata or get_release_metadata(None)
    candidates = (
        metadata.get("ref", ""),
        resolve_deploy_ref(),
        _image_tag(),
