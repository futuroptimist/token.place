...
def test_healthz_real_valkey_is_ready_without_capacity_or_state_mutation(
    valkey_server, monkeypatch
):
    namespace = uuid.uuid4().hex
    ...
    isolated_key = f"tokenplace:{{test:{other_namespace}}}:relay:v1:sentinel"
    try:
        ...
        assert tuple(client.dump(key) for key in exact_keys) == before
        assert client.get(isolated_key) == b"untouched"
    finally:
...
def test_expired_queue_reclaims_exact_stream_and_lifecycle_capacity(
    ...
    try:
        ...
        seconds, micros = store._foundation.server_time()
        deadline = seconds + micros / 1_000_000 + 0.25
        selected = store.select_and_reserve(
        ...
        if state != "reserved":
            store.enqueue_encrypted_request(
                "queued-client", "queued-request", selected.reservation_token, "node",
                "qwen3-8b-instruct", "8k-fast", deadline, envelope, "cancel",
            )
        if state == "claimed":
            store.claim_queued_request("node", owner, "consumer")
        unrelated = store.select_and_reserve(
            "unrelated-client", "unrelated-request", "qwen3-8b-instruct",
            "8k-fast", time.time() + 30,
        )
        time.sleep(0.3)

        if trigger == "select":
            store.select_and_reserve(
                "third-client", "third-request", "qwen3-8b-instruct",
                "8k-fast", time.time() + 30,
            )
        else:
            store.enqueue_encrypted_request(
                "unrelated-client", "unrelated-request",
                unrelated.reservation_token, "node", "qwen3-8b-instruct",
                "8k-fast", unrelated.request_deadline_epoch, envelope, "cancel",
            )

        terminal = store._foundation._client.hgetall(
            cfg.key("terminal", client, request)
        )
        assert terminal[b"outcome"] == b"expired"
        assert terminal[b"reason"] == b"request_deadline_expired"
        assert terminal[b"retrieval_state"] == b"completed_unavailable"
        expected_queue = 1 if trigger == "enqueue" else 0
        assert store._foundation._client.xlen(cfg.key("queue", node)) == expected_queue
        assert store._foundation._client.zscore(cfg.key("requests:deadline"), member) is None
        assert not store._foundation._client.exists(cfg.key("claim", client, request))
        assert store._foundation._client.zscore(cfg.key("claims:expiry"), member) is None
    finally:
        other_client = hashlib.sha256(b"client\0unrelated-client").hexdigest()
        other_request = hashlib.sha256(b"request\0unrelated-request").hexdigest()
        third_client = hashlib.sha256(b"client\0third-client").hexdigest()
        third_request = hashlib.sha256(b"request\0third-request").hexdigest()
        keys = [
            cfg.key("schema"), cfg.key("nodes:lease"), cfg.key("cursor"),
            cfg.key("reservations:expiry"), cfg.key("requests:deadline"),
            cfg.key("claims:expiry"), cfg.key("terminals:expiry"),
            cfg.key("control:expiry"), cfg.key("responses:expiry"),
            cfg.key("node", node), cfg.key("queue", node),
        ]
        for c, r in (
            (client, request),
            (other_client, other_request),
            (third_client, third_request),
        ):
            keys.extend((
                cfg.key("request", c, r), cfg.key("claim", c, r),
                cfg.key("terminal", c, r), cfg.key("response", c, r),
                cfg.key("progress", c, r), cfg.key("control", node, c, r),
            ))
        if "selected" in locals():
            keys.append(cfg.key("reservation", _digest(selected.reservation_token)))
        if "unrelated" in locals():
            keys.append(cfg.key("reservation", _digest(unrelated.reservation_token)))
        store._foundation._client.delete(*keys)
        store.close()
...
def test_expired_addressed_reservation_is_reclaimed_beyond_cleanup_batch(
    ...
    try:
        ...
        assert store._foundation._client.exists(
        ...
        terminal = store._foundation._client.hgetall(
            cfg.key("terminal", target[2], target[3])
        )
        assert terminal[b"outcome"] == b"expired"
        assert terminal[b"reason"] == b"request_deadline_expired"
        assert store._foundation._client.zcard(cfg.key("requests:deadline")) == 2
        assert store._foundation._client.zcard(cfg.key("reservations:expiry")) == 2
        assert store._foundation._client.hgetall(cfg.key("cursor")) == cursor_before
    finally:
        keys = [
            cfg.key("schema"), cfg.key("nodes:lease"), cfg.key("cursor"),
...
def test_progress_retrieval_rejects_invalid_envelope_without_mutation(
    ...
    node_id, owner, consumer = (
        ...
        _digest("invalid-progress-owner"),
        "worker",
    )
    identity = ("invalid-progress-client", "invalid-progress-request")
    try:
        store.register(node_id, _capabilities(), owner)
        seconds, micros = store._foundation.server_time()
        deadline = seconds + micros / 1_000_000 + 10
        ...
        progress_key = cfg.key("progress", client, request)
        store._foundation._client.hset(progress_key, "envelope", stored_envelope)

        def lifecycle_snapshot():
            client_api = store._foundation._client
            return (
                client_api.hgetall(progress_key),
                client_api.zscore(cfg.key("progress:expiry"), member),
                client_api.hgetall(cfg.key("request", client, request)),
                client_api.hgetall(cfg.key("claim", client, request)),
                client_api.zscore(cfg.key("requests:deadline"), member),
                client_api.zscore(cfg.key("claims:expiry"), member),
                client_api.xrange(cfg.key("queue", node)),
                client_api.zrange(cfg.key("node_work", node), 0, -1, withscores=True),
            )
        ...
        with pytest.raises(
            ...
            store.retrieve_encrypted_response(*identity, selection.reservation_token)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        b'{"cipherkey":"key","ciphertext":"progress","extra":"field","iv":"iv","protocol":"tokenplace_api_v1_relay_e2ee","version":1}',
        b'["key","progress","iv","tokenplace_api_v1_relay_e2ee",1]',
        b'{"cipherkey":"key","ciphertext":"progress","iv":"iv","protocol":"tokenplace_api_v1_relay_e2ee","version":2}',
        b'{"cipherkey":"key","ciphertext":"\xff","iv":"iv","protocol":"tokenplace_api_v1_relay_e2ee","version":1}',
    ),
    ids=("malformed", "noncanonical", "extra-field", "numeric-keys", "invalid-value", "invalid-utf8"),
)
@pytest.mark.parametrize(
    "transition", ("control", "cancel", "response", "claim", "progress", "node")
)
def test_progress_mutations_reject_invalid_envelopes_without_mutation(
    valkey_server, stored_envelope, transition
):
    store = _registration_store(valkey_server, uuid.uuid4().hex, claim_ttl_seconds=30)
    node_id, owner, consumer = "canonical-progress-node", _digest("canonical-progress-owner"), "worker"
    identity = ("canonical-progress-client", "canonical-progress-request")
    try:
        store.register(node_id, _capabilities(), owner)
        ...
        def snapshot():
            ...
            return (
                ...
                tuple(client_api.zrange(key, 0, -1, withscores=True) for key in indexes),
                client_api.xrange(cfg.key("queue", node)),
            )
        ...
        with pytest.raises(ValkeySchemaIncompatibleError, match="^state schema incompatible$"):
            if transition == "control":
                ...
                store.claim_queued_request(node_id, owner, "replacement-worker")
                ...
                store.replace_encrypted_progress_if_claimed(
                    ...
                    *identity,
                    claim.generation,
                    EncryptedProgressEnvelope(
                        "tokenplace_api_v1_relay_e2ee", 1, "replacement", "key", "iv"
                    ),
                )
            else:
                store.unregister_node_and_transition_work(node_id, owner)
        assert snapshot() == before
    finally:
        _delete_claim_fixture_state(store, (node_id,), (identity,))
        store.close()
...
def _force_retained_authority_due(store, keys, *, completed=False):
    ...
    if client.hget(keys["terminal"], "outcome") == b"expired":
        deadline = str(now - 4)
        client.hset(keys["request"], "deadline", deadline)
        if client.exists(keys["control"]):
            client.hset(keys["control"], "deadline", deadline)
    if completed:
        client.hset(
            keys["response"],
            mapping={
...
def test_node_pending_recovery_orphan_index_fences_registration_and_renewal(
    ...
    try:
        ...
        before = store._foundation._client.zrange(
            cfg.key("node_transitions:pending"), 0, -1, withscores=True
        )
        with pytest.raises(ValkeySchemaIncompatibleError):
            store.expire()
        assert store._foundation._client.zrange(
            cfg.key("node_transitions:pending"), 0, -1, withscores=True
        ) == before
...
@pytest.mark.parametrize(
    ("field", "value"),
    (("node_id", "different-node"), ("transition_epoch", "")),
)
def test_node_pending_recovery_rejects_invalid_identity_before_dispatch(
    valkey_server, monkeypatch, field, value
):
    store = _registration_store(
        valkey_server, uuid.uuid4().hex, node_transition_batch_size=1
    )
    node, owner = "pending-invalid", _digest("pending-invalid-owner")
    identities = (("pending-invalid-client-1", "request-1"), ("pending-invalid-client-2", "request-2"))
    cfg, digest = store._foundation.config, store._node_digest(node)
    try:
        store.register(node, _capabilities(), owner)
        seconds, micros = store._foundation.server_time()
        ...
        store._foundation._client.hset(
            cfg.key("node_transition", digest), field, value
        )
        keys = (
            cfg.key("node_transition", digest),
            cfg.key("node_transitions:pending"),
        ...
        before = (
            store._foundation._client.hgetall(keys[0]),
            store._foundation._client.zrange(keys[1], 0, -1, withscores=True),
            store._foundation._client.zrange(keys[2], 0, -1, withscores=True),
        )
        monkeypatch.setattr(
            store,
            "unregister_node_and_transition_work",
            lambda *args, **kwargs: pytest.fail("invalid discovery was dispatched"),
        )
        with pytest.raises(ValkeySchemaIncompatibleError):
            store.expire()
        assert (
...
def test_node_pending_recovery_leaves_not_yet_due_transition_untouched(valkey_server):
    store = _registration_store(
        valkey_server, uuid.uuid4().hex, node_transition_batch_size=1
    )
    node, owner = "pending-future", _digest("pending-future-owner")
    identities = (("pending-future-client-1", "request-1"), ("pending-future-client-2", "request-2"))
    cfg, digest = store._foundation.config, store._node_digest(node)
    try:
        store.register(node, _capabilities(), owner)
        seconds, micros = store._foundation.server_time()
        ...
        store._foundation._client.zadd(
            cfg.key("node_transitions:pending"), {digest: now + 60}
        )
        keys = (cfg.key("node_transition", digest), cfg.key("node_transitions:pending"), cfg.key("node_work", digest))
        before = (
            store._foundation._client.hgetall(keys[0]),
            store._foundation._client.zrange(keys[1], 0, -1, withscores=True),
            store._foundation._client.zrange(keys[2], 0, -1, withscores=True),
...
def test_node_transition_capacity_accepts_post_deadline_controls_live_and_due(
    valkey_server, outcome
):
    namespace = uuid.uuid4().hex
    writer = _registration_store(valkey_server, namespace)
    observer = _registration_store(valkey_server, namespace)
    retained_node, owner = "capacity-post-deadline", _digest("capacity-post-deadline-owner")
    retained = ("capacity-post-deadline-client", "retained")
    targets = (("capacity-target-live", "live"), ("capacity-target-due", "due"))
    try:
        writer.register(retained_node, _capabilities(), owner)
        _enqueue_claim_fixture(writer, retained_node, owner, *retained, time.time() + 60)
        claim = writer.claim_queued_request(retained_node, owner, "retained-consumer")
        keys = _retained_authority_keys(writer, retained_node, retained)
        client, request = writer._identity(*retained)
        claim_key = writer._foundation.config.key("claim", client, request)
        past = time.time() - 2
        ...
        if outcome == "expiry":
            result = writer.cancel_or_expire_request(
                *retained, status="expired", reason="request_deadline_expired"
            )
            assert result.new_outcome
            removed = writer.unregister_node_and_transition_work(retained_node, owner)
            assert removed.state == "complete" and removed.processed_count == 0
        else:
            removed = writer.unregister_node_and_transition_work(retained_node, owner)
            assert removed.state == "complete" and removed.claims_terminalized == 1
        assert claim.generation > 0

        for target_node, suffix in targets:
            target_owner = _digest(f"capacity-target-owner-{suffix}")
            identity = ("capacity-target-client", suffix)
            writer.register(target_node, _capabilities(), target_owner)
            writer.select_and_reserve(
                *identity, "qwen3-8b-instruct", "8k-fast", time.time() + 60
            )
            result = observer.unregister_node_and_transition_work(target_node, target_owner)
            assert (result.state, result.processed_count, result.new_outcomes) == (
                "complete", 1, 1
            )
            if suffix == "live":
                assert writer._foundation._client.exists(keys["terminal"]) == 1
                _force_retained_authority_due(writer, keys)
            else:
                assert writer._foundation._client.exists(keys["terminal"]) == 0
                assert writer._foundation._client.exists(keys["control"]) == 0
    finally:
        _delete_claim_fixture_state(
            observer,
            (retained_node, *(node for node, _ in targets)),
            (retained, *(("capacity-target-client", suffix) for _, suffix in targets)),
        )
        observer.close()
        writer.close()
...
def test_node_transition_capacity_rejects_completed_response_after_deadline_without_mutation(
    ...
    writer = _registration_store(valkey_server, namespace)
    observer = _registration_store(valkey_server, namespace)
    retained_node, target_node = "capacity-completed", f"capacity-target-{retained_state}"
    retained_owner, target_owner = _digest("capacity-completed-owner"), _digest("capacity-target-owner")
    valid_identity = ("capacity-valid-client", "cleanup")
    invalid_identity = ("capacity-invalid-client", retained_state)
    target_identity = ("capacity-target-client", retained_state)
    try:
        writer.register(retained_node, _capabilities(concurrency=3), retained_owner)
        _enqueue_claim_fixture(
            writer, retained_node, retained_owner, *invalid_identity, time.time() + 60
        )
        claim = writer.claim_queued_request(retained_node, retained_owner, "completed-consumer")
        writer.accept_encrypted_response(
            retained_node,
            retained_owner,
            "completed-consumer",
            *invalid_identity,
        ...
        invalid = _retained_authority_keys(writer, retained_node, invalid_identity)
        writer.select_and_reserve(
            *valid_identity, "qwen3-8b-instruct", "8k-fast", time.time() + 60, "cancel"
        )
        writer.cancel_or_expire_request(*valid_identity, "cancel")
        valid = _retained_authority_keys(writer, retained_node, valid_identity)
        writer.register(target_node, _capabilities(), target_owner)
        writer.select_and_reserve(
            *target_identity, "qwen3-8b-instruct", "8k-fast", time.time() + 60
        )
        accepted = float(writer._foundation._client.hget(invalid["terminal"], "accepted_at_epoch"))
        writer._foundation._client.hset(invalid["request"], "deadline", str(accepted - 1))
        if retained_state == "due":
            _force_retained_authority_due(writer, invalid, completed=True)
            accepted = float(writer._foundation._client.hget(invalid["terminal"], "accepted_at_epoch"))
            writer._foundation._client.hset(invalid["request"], "deadline", str(accepted - 1))

        _force_retained_authority_due(writer, valid)
        extra = tuple(
            authority[name]
            for authority in (valid, invalid)
            for name in (
                "request", "response", "terminal", "control", "response_index",
                "terminal_index", "control_index", "deadline_index",
            )
        )
        before = _node_transition_authority_snapshot(
        ...
        assert writer._foundation._client.exists(valid["terminal"]) == 1
    finally:
        _delete_claim_fixture_state(
            observer,
            (retained_node, target_node),
            (valid_identity, invalid_identity, target_identity),
        )
        observer.close()
        writer.close()
