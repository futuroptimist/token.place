...
|-----------------|--------------|--------------------------------------------------------------------|
| API_RATE_LIMIT  | 60/hour      | Per-IP rate limit for user-facing API requests; API v1 compute-node control-plane routes use separate budgets; `GET`/`HEAD` requests to `/`, `/api/v1/meta`, and `/api/v1/version` do not consume this quota |
...
| API_DAILY_QUOTA | 1000/day     | Per-IP daily request quota; `GET`/`HEAD` requests to `/`, `/api/v1/meta`, and `/api/v1/version` do not consume this quota |
