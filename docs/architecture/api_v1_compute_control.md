...
The relay establishes one authoritative request deadline when a request is admitted to `/api/v1/relay/requests`. The default is 480 seconds and is configurable via relay environment variables while remaining bounded. Browser and distributed-provider waits add only five seconds of response-propagation grace, so they cannot preempt the relay-owned deadline. The deadline is stored with queued and in-flight request state, returned in admission metadata, and dispatched with the encrypted request envelope as relative TTL metadata.
