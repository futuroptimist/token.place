...
- `tokenplace_public_rate_limit_requests_total`: counter for every public HTTP
  quota decision, independent of relay inference outcomes. Its labels are the
  fixed `route_class`, `method`, `outcome`, and `rejection_reason` enums below.
...
preserved with its prior `method`, `endpoint`, and `status` label schema so
existing scrapes and tests continue to work while new dashboards migrate to the
bounded `tokenplace_http_requests_total` contract. Its method values now use the
same fixed enum rather than an arbitrary request method. Relay metrics use a
dedicated `CollectorRegistry`; `api.init_app(...)` accepts narrow optional metrics
...
- `method` for canonical HTTP metrics: `GET`, `POST`, `PUT`, `PATCH`,
  ...
  `tokenplace_relay_requests_total{method,endpoint,status}` counter uses this
  enum as well.
- `route_class` for public quota telemetry: `root`, `public_metadata`,
  `public_version`, `api_v1_inference`, `api_v1_other`, `api_v2`,
  `relay_control`, `health`, `metrics`, `static`, `unknown`. Classification is
  derived from Flask's application-owned endpoint names; every unmatched path
  maps to exactly `unknown`.
- `outcome` for public quota telemetry: `accepted`, `exempt`, `rejected`.
- `rejection_reason`: `none`, `hourly`, `daily`, `other`. Accepted and exempt
  samples always use `none`; rejected samples use one of the other three.
  Unknown limiter windows and application-generated 429 responses map to
  `other`, never to exception or limiter text.
...
over windows and account for restart resets. The supported container and Helm
configuration pins `RELAY_WORKERS=1`; the dedicated registry does not enable
the Prometheus client's multiprocess-file collector. If an operator overrides
that setting, each Gunicorn worker exposes only its own counters and a scrape
can reach any worker, so sums are incomplete and discontinuous. Multi-worker
aggregation is not supported until the runtime explicitly configures and tests
the Prometheus multiprocess collector and worker cleanup. Focused tests launch
separate processes to prove counters do not leak or aggregate between workers.
...
Public quota telemetry has no client-level remaining-budget gauge. Flask-Limiter
tracks budgets per limiter key, so aggregating remaining values would require
enumerating identity state or inventing ambiguous cross-client minima. The
outcome/rejection counter is therefore the supported pressure signal: it is
aggregate-only, counts accepted and exempt traffic as context, and exposes
hourly and daily exhaustion without retaining identities.

The finite upper bound is 440 realizable series: 11 route classes times 8
methods times 2 non-rejected outcomes with `rejection_reason="none"` (176), plus
11 times 8 times 1 rejected outcome times 3 rejection reasons (264). Raw paths,
queries, network or forwarded addresses, limiter keys, identities, credentials,
tokens, request IDs, and exception text cannot affect that bound or exposition.
...
sum by (route_class, rejection_reason) (rate(tokenplace_public_rate_limit_requests_total{outcome="rejected"}[5m]))
sum by (route_class) (rate(tokenplace_public_rate_limit_requests_total{outcome=~"accepted|exempt"}[5m]))
...
Sugarkube #2405 should consume the first public-quota query for rejection-rate
alerts and panels, split only by `route_class` and `rejection_reason`; it must
not join identity-bearing data or manufacture raw-path labels. Sugarkube #2782
may retain windowed `increase()` results over those same bounded dimensions,
plus aggregate accepted/exempt denominators. Neither consumer should retain raw
scrape payloads as incident evidence when the bounded aggregate is sufficient.
