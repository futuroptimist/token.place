...
def test_qwen_64k_runtime_fails_when_yarn_enum_missing_and_constructor_lacks_rope_scaling(tmp_path):
    ...
    class FakeLlama:
        def __init__(self, model_path, n_gpu_layers, n_ctx, verbose, rope_freq_scale, yarn_orig_ctx):
            pass
        def apply_chat_template(self, messages, tokenize=False, add_generation_prompt=True, enable_thinking=False):
            return '<qwen>'
...
def test_qwen_64k_missing_reason_does_not_repeat_rope_scaling_type():
    ...
    assert 'runtime_qwen_64k_yarn_rope_freq_scale_unavailable' in diagnostics['missing_reason'] and 'missing constructor kwargs: rope_freq_scale, yarn_orig_ctx' in diagnostics['missing_reason']
