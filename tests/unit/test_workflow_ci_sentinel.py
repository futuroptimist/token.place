...
DESKTOP_RELEASE_WORKFLOW = WORKFLOW_DIR / "desktop-release.yml"
...
def test_desktop_release_workflow_is_tag_or_manual_only() -> None:
    workflow_data = _load_workflow(DESKTOP_RELEASE_WORKFLOW)
    on_block = _workflow_on_block(workflow_data, "desktop-release.yml")

    assert "pull_request" not in on_block, (
        "desktop-release.yml must not run full release packaging on ordinary PRs"
    )
    assert on_block["push"]["tags"] == ["desktop-v*"]
    assert "workflow_dispatch" in on_block


def test_desktop_release_workflow_cancels_duplicate_release_builds() -> None:
    workflow_data = _load_workflow(DESKTOP_RELEASE_WORKFLOW)
...
def test_desktop_release_temporary_artifacts_expire_after_seven_days() -> None:
    ...
    assert all(step.get("with", {}).get("retention-days") == "7" for step in upload_steps)
