...
- `method` for HTTP metrics: `GET`, `POST`, `PUT`, `PATCH`, `DELETE`,
  `OPTIONS`, `HEAD`, `other`. The compatibility
  `tokenplace_relay_requests_total{method,endpoint,status}` counter retains its
  label names and established values for recognized methods while collapsing
  unrecognized caller-controlled methods to `other`.
...
### Public quota pressure contract

`tokenplace_public_http_quota_outcomes_total` counts every relay HTTP request
independently of inference terminal outcomes. Its labels are closed enums:

- `route_class`: `root`, `public_metadata`, `public_version`, `public_api`,
  `client_relay_read`, `compute_control_plane`, `operational`, or
  `unknown_route`. The first three are exact application-owned paths; API and
  relay classes come from Flask endpoint ownership and reviewed exact-path
  sets. Every unmatched path collapses to `unknown_route`.
- `method`: `GET`, `POST`, `PUT`, `PATCH`, `DELETE`, `OPTIONS`, `HEAD`, or
  `other`.
- `outcome`: `accepted`, `exempt`, or `rejected`. `accepted` means the public
  quota admitted the request, not that the handler returned success. `exempt`
  means application policy bypassed the public quota. `rejected` is emitted
  only by Flask-Limiter's public-quota exception handler.
- `reason`: `none`, `hourly_limit`, `daily_limit`, or `other_limit`.
  Non-rejections always use `none`; an unrecognized limiter window fails into
  `other_limit` without exporting the limiter description or exception.

Only valid combinations are emitted: accepted/exempt use `reason="none"`, and
rejected uses one of the three limit reasons. The hard upper bound is therefore
`8 route classes * 8 methods * (2 non-rejected + 3 rejected reasons) = 320`
counter series per process. Raw paths, query strings, direct or forwarded
addresses, limiter keys, client identity, credentials, tokens, request IDs,
and exception text are neither labels nor help text.

The counter deliberately does not expose remaining budget. Flask-Limiter's
budget is per client key; aggregating its storage would require iterating or
retaining client-level state and could leak population/identity information.
The bounded accepted/exempt/rejection counter is the supported safe pressure
signal. This also avoids implying that process-local in-memory budgets are a
single global remaining balance.

The supported container contract is Gunicorn with `RELAY_WORKERS=1` and four
threads. The dedicated registry and counter are shared safely by threads in
that process. They reset on worker/pod restart and `/metrics` exposes the
current worker's aggregate since its start. The repository does not configure
`PROMETHEUS_MULTIPROC_DIR` or `MultiProcessCollector`; do not raise the worker
count and sum process-local metrics as though they represented a coordinated
quota. A future supported multi-worker runtime must add shared relay/limiter
state and explicit Prometheus multiprocess lifecycle handling together.

For Sugarkube #2405, consume rejection pressure with
`sum by (route_class, reason) (rate(tokenplace_public_http_quota_outcomes_total{outcome="rejected"}[5m]))`
and compare it with admitted traffic using the same counter filtered to
`outcome="accepted"`. Use `outcome="exempt"` for root/metadata/version probe
parity; do not infer exemptions from status codes. Thresholds remain
Sugarkube-owned and must be qualified in staging.

For Sugarkube #2782, retain only windowed `sum by (route_class, method,
outcome, reason)` aggregates derived from this metric plus the release/time
context already approved there. Do not retain raw exposition, per-pod snapshots
as identity evidence, HTTP logs, headers, client keys, or reconstructed limiter
buckets. Counter resets must be handled with `increase()`/`rate()` and recorded
as a restart boundary, not repaired by carrying client-level state forward.
...
## Implemented canonical chart scrape contract slice

The canonical Sugarkube chart at `charts/tokenplace` now has an opt-in, authenticated
Prometheus scrape contract. Metrics and `ServiceMonitor` rendering remain disabled by
default for existing deployments. When operators set `metrics.enabled=true` and provide
`metrics.auth.existingSecret`, the chart injects `TOKENPLACE_METRICS_TOKEN` via a
`SecretKeyRef`; no plaintext token is stored in chart values, templates, docs examples,
or tests. When `serviceMonitor.enabled=true`, the
chart renders a `monitoring.coreos.com/v1` `ServiceMonitor` selecting the canonical
token.place Service by its `app.kubernetes.io/name` and `app.kubernetes.io/instance`
labels, scraping the named `http` port at the relay's fixed `/metrics` route.

The `ServiceMonitor` uses the Prometheus Operator `authorization.credentials` Secret
reference rather than deprecated bearer-token file fields. The Secret must exist in the
same namespace as the `ServiceMonitor` and be readable by the Prometheus stack. Sugarkube
release discovery is configurable through `serviceMonitor.additionalLabels`; the default
keeps `release: kube-prometheus-stack`, matching the current platform selector without
making the selector irreversible. Chart-owned identity labels are merged with
additional labels and win collisions, so overlapping keys do not produce duplicate YAML
mapping keys. Bounded metric relabeling hooks are configured with
`serviceMonitor.relabelings` and limited to fixed `app`, `environment`, `release`,
and `cluster` labels.

This chart slice intentionally does not add a dedicated public `/metrics` Ingress path.
The existing catch-all `/` Prefix Ingress may still route `/metrics` when metrics are
enabled, so bearer authentication must deny unauthenticated public requests. The slice
does not copy the legacy `deploy/charts/tokenplace-relay` monitoring resources wholesale
and does not alter accepted relay architecture constraints: one replica, one Gunicorn
worker, `Recreate` rollout, and in-memory relay state remain the canonical Sugarkube
runtime contract for this phase.

### Chart scrape verification commands
...
helm template tokenplace charts/tokenplace --namespace tokenplace \
  ...
  --set serviceMonitor.relabelings.release=main-REPLACE_SHORTSHA \
  --set serviceMonitor.relabelings.cluster=sugarkube-staging
helm lint charts/tokenplace
```

After deployment, prove internal scrape success, public denial, release discovery, and
bounded target labels with these checks. They verify the Secret without printing it,
confirm the Prometheus Operator service account can read it, assert the discovered
target is healthy with the four configured labels, fail if public unauthenticated
`/metrics` returns a successful Prometheus payload, and record chart/image identity.
...
kubectl auth can-i get secret/"$SECRET" \
  --namespace "$NS" \
  --as "system:serviceaccount:${PROM_NS}:${PROM_SA}"
kubectl -n "$PROM_NS" exec "$PROM_POD" -- wget -qO- \
  'http://127.0.0.1:9090/api/v1/targets?state=active' | \
  python3 -c 'import json,sys; data=json.load(sys.stdin)["data"]["activeTargets"]; matches=[t for t in data if t.get("labels",{}).get("app") == "tokenplace" and t.get("labels",{}).get("environment") and t.get("labels",{}).get("release") and t.get("labels",{}).get("cluster")]; assert matches and all(t["health"] == "up" for t in matches), matches'
