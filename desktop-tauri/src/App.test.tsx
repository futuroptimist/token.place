...
describe("desktop app start failure handling", () => {
  ...
  it("renders operator debug log affordances from live events and opens in-app log text", async () => {
    invokeMock.mockImplementation((command: string) => {
      if (command === "read_operator_log") {
        return Promise.resolve(
          "desktop.compute_node.stderr bridge stderr line",
        );
      }
      if (
        command === "reveal_operator_log" ||
        command === "open_operator_debug_terminal"
      ) {
        return Promise.resolve(undefined);
      }
    ...
    const openLogButton = (await screen.findByText(
      "Open debug log",
    )) as HTMLButtonElement;
  ...
  it("clears stale operator log path when backend emits null", async () => {
    mockInitialComputeStatus({
      ...
      active_relay_url: "https://token.place",
      log_file_path:
        "/Users/Example User/Library/Logs/token.place/operator/compute-node-old.log",
      operator_session_id: "session-1",
      sequence: 1,
    });
    ...
    render(<App />);
    await waitFor(() =>
      expect(screen.getByText(/Operator debug log:/).textContent).toContain(
        "compute-node-old.log",
      ),
    );
    ...
    expect(
      (screen.getByText("Open debug log") as HTMLButtonElement).disabled,
    ).toBe(true);
  });
  ...
  it("copies operator log path and surfaces path copy failures", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText(/path clipboard denied/)).toBeTruthy(),
    );
    Object.defineProperty(navigator, "clipboard", {
      configurable: true,
  ...
  it("handles Start Stop Start with fresh registration and cleared errors", async () => {
    ...
    fireEvent.click(
      (await screen.findByText("Stop operator")) as HTMLButtonElement,
    );
    await waitFor(() =>
      expect(
        invokeMock.mock.calls.some(
  ...
  it("renders the full initial idle compute-node status contract", async () => {
    ...
    expect(screen.getByText(/Effective mode:/).textContent).toContain(
      "pending",
    );
    expect(screen.getByText(/Backend available:/).textContent).toContain(
      "pending",
    );
    expect(screen.getByText(/Backend selected:/).textContent).toContain(
      "pending",
    );
  ...
  it("merges compute-node readiness diagnostics into the desktop status snapshot", async () => {
    ...
    expect(
      screen.getByText(/Readiness diagnostics:/).textContent,
    ).not.toContain("unsafe_nested");
    expect(
      screen.getByText(/Readiness diagnostics:/).textContent,
    ).not.toContain("api_v1_readiness_completion_smoke_internal_reason");
    expect(
      screen.getByText(/Readiness diagnostics:/).textContent,
  ...
  it("ignores stale compute-node events from a prior operator session", async () => {
    ...
    expect(screen.getByText(/Last error:/).textContent).toContain("none");
    expect(
      screen.queryByText("old error side effect after restart"),
    ).toBeNull();
  });
  ...
  it("renders one relay URL field from legacy config and migrates it on save", async () => {
    ...
    const relayInput = (await screen.findByLabelText(
      "Relay URL 1",
    )) as HTMLInputElement;
  ...
  it("loads and displays multiple persisted relay URLs", async () => {
    ...
    expect(
      ((await screen.findByLabelText("Relay URL 1")) as HTMLInputElement).value,
    ).toBe("https://token.place");
    expect(
      ((await screen.findByLabelText("Relay URL 2")) as HTMLInputElement).value,
    ).toBe("https://staging.token.place");
    expect(screen.getByText(/Configured relay URLs:/).textContent).toContain(
      "https://token.place, https://staging.token.place",
    );
  });
  ...
  it("adds and removes relay URL fields while keeping one field", async () => {
    ...
    const secondRelayInput = (await screen.findByLabelText(
      "Relay URL 2",
    )) as HTMLInputElement;
    expect(secondRelayInput.value).toBe("");
    fireEvent.change(secondRelayInput, {
      target: { value: " https://staging.token.place " },
    });

    const deleteSecondRelayButton =
      await screen.findByLabelText("Delete relay URL 2");
  ...
  it("enforces the documented maximum of 10 relay URL fields", async () => {
    ...
    for (let relayNumber = 2; relayNumber <= 10; relayNumber += 1) {
      ...
      fireEvent.click(addButton);
      expect(
        await screen.findByLabelText(`Relay URL ${relayNumber}`),
      ).toBeTruthy();
    }
  ...
  it("persists 64k context tier while stopped, displays selected window, and starts by stable profile ID only", async () => {
    ...
    render(<App />);
    const contextSelect = (await screen.findByLabelText(
      "Context tier",
    ...
    await waitFor(() =>
      expect(screen.getByText(/Context window:/).textContent).toContain(
        "65536",
      ),
    );
    await waitFor(() =>
  ...
  it.each([
    ...
    [
      "warming",
      { running: false, worker_state: "warming", warm_load_state: "warming" },
      true,
    ],
    [
      "running",
      { running: true, worker_state: "ready", relay_runtime_state: "ready" },
      true,
    ],
    [
      "stopping",
    ...
    "sets context tier disabled=%s while operator is %s",
    async (_label, statusOverrides, expectedDisabled) => {
      ...
      await waitFor(() =>
        expect(contextSelect.disabled).toBe(expectedDisabled),
      );
    },
  );
  ...
  it("renders worker lifecycle fields and ignores stale worker generations", async () => {
    ...
    expect(computeHandler).toBeTruthy();
    await waitFor(() =>
      expect(screen.getByText(/Worker state:/).textContent).toContain(
        "stopped",
      ),
    );
    ...
    expect(screen.getByText(/Worker restart count:/).textContent).toContain(
      "1",
    );
    ...
    expect(screen.getByText(/Last worker error code:/).textContent).toContain(
      "worker_dead",
    );
  ...
  it("re-enables context tier and relay controls immediately after successful Stop Operator", async () => {
    ...
    const addRelayButton = (await screen.findByText(
      "Add new relay URL",
    )) as HTMLButtonElement;
  ...
  it("forces stopped worker fields even when stopped events include ready lifecycle values", async () => {
    ...
    computeHandler?.({
      payload: {
        ...
        relay_statuses: [
          { relay_url: "https://token.place", registered: true },
        ],
        operator_session_id: "session-1",
        sequence: 3,
      },
  ...
  it("re-enables context tier after pre-registration failure event", async () => {
    ...
    await waitFor(() => expect(contextSelect.disabled).toBe(false));
    expect(screen.getByText(/Last error:/).textContent).toContain(
      "registration failed before ready",
    );
  });
  ...
  it("normalizes packaged macOS Python stub diagnostics and disables Python actions", async () => {
    const raw =
      "no usable Python 3 interpreter found for desktop Python subprocess (consulted override env var: TOKEN_PLACE_PYTHON); tried: python3  -> status=1 stdout='' stderr='xcode-select: note: No developer tools were found, requesting install. If developer tools are located at a non-default location on disk, use sudo xcode-select --switch /Applications/Xcode.app'; python  -> spawn failed: No such file or directory; /Users/alice/.pyenv/shims/python";
    invokeMock.mockImplementation((command: string) => {
      if (command === "detect_backend") {
        return Promise.resolve({
          platform_label: "macos",
    ...
    const error = await screen.findByText(
      /The bundled token\.place runtime is missing or damaged/,
    );
    expect(error.textContent).toContain(
      "Diagnostic code: desktop_python_runtime_missing",
    );
    expect(error.textContent).not.toContain("xcode-select");
    expect(error.textContent).not.toContain("sudo");
    expect(error.textContent).not.toContain("Xcode.app");
    expect(error.textContent).not.toContain("/Users/alice");
    expect(error.textContent).not.toContain("stderr");
    expect(
      ((await screen.findByText("Download")) as HTMLButtonElement).disabled,
    ).toBe(true);
    expect(
      ((await screen.findByText("Start operator")) as HTMLButtonElement)
        .disabled,
    ).toBe(true);
    const promptArea = screen
      .getByText("Prompt")
      .parentElement?.querySelector("textarea");
    fireEvent.change(promptArea as HTMLTextAreaElement, {
      target: { value: "hello" },
    ...
    expect(
      ((await screen.findByText("Start local inference")) as HTMLButtonElement)
        .disabled,
    ).toBe(true);
  });
});
