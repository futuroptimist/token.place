...
## Read-only qualification of an existing index

`.github/workflows/qualify-relay-oci-index.yml` is a manual, read-only executor for an immutable
index that is already present in `ghcr.io/futuroptimist/tokenplace-relay`. It checks out the gate
from `main`, records that gate implementation commit independently from the artifact source, resolves
exactly one `linux/amd64` and one `linux/arm64` descriptor from the supplied index, pulls those
descriptor digests, and runs the gate once per platform. It never builds, publishes, promotes,
retags in the registry, or accesses a deployment environment. Inputs are a full lowercase
`sha256:` index digest, a full lowercase 40-character source commit, bounded release ref and base,
and a 1--48 character evidence label containing only letters, digits, `.`, `_`, and `-`.

The executor uploads one artifact even when qualification fails. It contains the sanitized bounded
`index-manifest.json`, `amd64-evidence.json`, `arm64-evidence.json`, bounded `metadata.json`, and
`SHA256SUMS`. A successful job requires both evidence reports to parse without duplicate keys,
contain every current contract result exactly once with a passing state, bind to the requested
source/index/platform descriptors, and report no gate or executor cleanup failure. Reports retain
only the gate's privacy-safe aggregate checks and immutable identities; they do not contain
credentials, headers, paths generated by the cardinality probe, bodies, responses, client
identities, cryptographic material, prompts, ciphertext, or model output.

The planned Step 05b dispatches below are documentation only. They have **not** been run or qualified:

| Artifact | `index_digest` | `source_commit` | `release_ref` | `release_base` | `evidence_label` |
| --- | --- | --- | --- | --- | --- |
| Qwen candidate | `sha256:b32ef19840dabe44caf7240b787af14dcf439b39357833e21a92c3dd511effd4` | `8618c9aba4b5dfe7980c2fe861095a92311145f2` | `main-8618c9a` | `main` | `step-05b-qwen-8618c9a` |
| Corrected Llama rollback | `sha256:543fde33aff45253630090b52d16163e3586da12c973f5c4a658ddc8927d0a68` | `6c39adc64e7bed4f85d07164aa2860e637919ca9` | `sha-6c39adc` | `release/relay-0.1.1` | `step-05b-llama-6c39adc` |
